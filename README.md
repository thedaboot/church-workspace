...
`api/` 서버 함수(AI·공유 미리보기·드라이브·유튜브)는 **`npm run dev`가 그대로 돌립니다** —
`vite.config.js`의 dev 전용 미들웨어가 `/api/<이름>` 요청을 `api/<이름>.js`에 넘기고 `.env`의
서버 키를 실어 줍니다(2026-09-05). 게스트 모드에는 붙지 않습니다.
...
- 모임 — 내 순(구성원·출석·순에 공유된 노트) · 동아리(가입 신청·모임·리더의 이름·설명 수정 · **신청 QR** —
  ...
  나눔 질문, 사용자 템플릿 그대로), 마스터가 고정하면 모두 그 가이드를 보며, 이미지(PNG)로 저장해 카카오톡으로 보냅니다.
...
- 참고 링크 — 기획안·시트 같은 외부 링크를 프로젝트 상단에 달아 둡니다(`+ 참고 링크`).
  **업무에서는 '첨부 파일' 구역 안**에 파일 줄과 한 목록으로 서고 버튼은 `+ 링크`입니다
  (2026-09-10 — 링크로 첨부하는 것이므로 첨부와 같은 자리입니다). 노션·유튜브·
...
- 첨부 파일 — 수정 모드에서 `+ 파일`·드래그앤드롭·붙여넣기로 올립니다(클라우드 모드).
  같은 구역의 `+ 링크`로 **구글 시트 같은 링크도 파일과 한 목록에** 답니다(아래 '참고 링크').
  파일은 한 파일 25MB,
...
- Gemini AI — 업무 3줄 요약, 본문 구조화 다듬기(`api/ai.js` 경유, `gemini-3.1-flash-lite`).
  **AI가 무엇을 보고 무엇을 쓰는지는 [`docs/AI.md`](docs/AI.md)가 기준입니다.**
  ...
  팀·직함과 함께 실립니다. 배경 지식으로 교회 달력(주일 4부 젊은이 예배의 순서 · 성찬 예배 ·
  ...
  **다듬기**는 회의록(강평회)도 다룹니다 — '정한 것 / 아직 정하지 못한 것 / 누가 무엇을
  언제까지'로 갈라 주고, 우리 표현(3층 본당 · 주일 4부 젊은이 예배 · 순 · 콘티 · 송폼)으로
...
│                        · links(링크 칩·링크 줄·추가 팝오버 — 프로젝트 헤더와 업무 창
│                          '첨부 파일' 구역이 공용으로 씁니다)
...
| `0039_sun_guides_and_perms.sql` | 순 편성 자격(마스터·관리자·리더순장) · 동아리 정보 수정(관리자·리더) · 순모임 가이드 `sun_guides` |
| `0040_people_sun_exempt.sql` | `people.sun_exempt` — 순 편성 대상이 아닌 사역자(부장·전도사). 이름을 코드에 박지 않으려고 둔 칸 |
| `0041_groups_unique_name.sql` | 같은 이름 막기 — 순은 같은 해 안에서만, 동아리는 전체(환송한 것은 제외) |
| `0042_service_edit_perms.sql` | `can_edit_service()` — 주보 작성·수정 자격을 관리자·회장·미디어팀으로 |
| `0043_people_roles_split.sql` | 직분을 `officer` 하나에서 다섯(부장·회장·총무·리더순장·리더팀장)으로. 교역자는 `people.is_pastor` |
| `0044_services_praise_leader.sql` | `services.praise_leader` — 찬양 인도자 이름 |
| `0045_perms_2026_09_05.sql` | 권한 다섯 손질 — 주보·순 편성에 교역자 복귀 · 전체 출석은 관리자·교역자·리더순장만 · 동아리 멤버는 관리자도 · 마스터가 남의 묵상 삭제 |
| `0046_services_praise_playlist.sql` | `services.praise_playlist_url` — 재생목록 주소를 `playlist?list=…`로 정규화해 저장 |
| `0047_service_files.sql` | 주보 송폼 — `files.service_id`(+`card_id`와 배타 CHECK) · `services.drive_folder_id` · `files` RLS 4개를 업무/주보 갈래로 |
| `0048_touch_last_seen_now.sql` | `touch_last_seen()` — 다녀간 시각을 기기 시계가 아니라 **서버 `now()`** 로 찍는다 |
...
| `0058_card_resource_links.sql` | 참고 링크를 업무(카드)에도 — `resource_links.card_id` + 프로젝트 축과 배타 CHECK. 업무에 붙인 구글 시트를 앱 안에서 열어 고친다(화면 자리는 업무 창의 '첨부 파일' 구역 — 2026-09-10) |
...
| `YOUTUBE_API_KEY` | 찬양 재생목록 가져오기 — 있으면 Data API로 전체, 없으면 RSS로 최신 15곡 | **서버 전용** |
