...
- **실시간 소통 & @멘션 자동완성**: 업무별 댓글 피드에서 `@`를 입력하면 팀 멤버 제안 목록이 떠서 ↑↓·Enter·클릭으로 고를 수 있습니다. 댓글, 작업 상세 내용, 담당자 입력에서 모두 동작하며, 담당자는 멤버를 칩(chip)으로 선택하거나 직접 이름을 입력해 추가할 수 있어요.
...
- **본문 이미지 붙여넣기**(클라우드 모드): 작업 상세 내용 입력창에 이미지를 붙여넣으면 공개 버킷(`content-images`)에 올라가고 커서 위치에 이미지가 삽입됩니다. 게스트 모드에서는 이미지 URL을 직접 붙여넣어 표시할 수 있어요.
