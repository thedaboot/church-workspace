...
- **프로젝트 캘린더**: 보드/캘린더 뷰를 전환하며 업무 일정을 달력에서 봅니다. 한 업무는 **한 줄로만** 나오고(제목은 시작 칸에 한 번), 기간은 시작~마감이 띠로 이어집니다. 색은 **담당 팀 색**을 따르고 여러 팀이 함께 하는 업무는 팀 색 줄무늬로 표시해요. 하루에 업무가 몰려 칸에 다 안 들어가면 **+N**으로 접고, 이때 **시작일이 빠른 업무가 위**로 옵니다. 날짜를 누르면 그 날 업무 전체가 아래 목록으로 펼쳐집니다(모바일은 칸이 좁아 팀 색 점으로 표시하고, 날짜를 탭해 목록에서 읽습니다). ‹ › 버튼으로 올해부터 2030년까지 월 단위로 이동하고, '오늘' 버튼으로 즉시 이번 달로 돌아올 수 있어요. 마감일은 노션 톤의 커스텀 데이트피커로 요일까지 함께 표시·선택합니다.
