...
- **첨부 미리보기**: 첨부를 누르면 스토리지 링크로 새 탭이 열리는 대신 **앱 안 미리보기 모달**이 뜹니다. 이미지·PDF·영상·소리·텍스트(txt·md·csv·json 등)는 브라우저가 직접 그리고(외부로 나가지 않아요), 워드·엑셀·파워포인트는 마이크로소프트 오피스 미리보기로 표시합니다(화면에 그 사실을 표기). hwp처럼 방법이 없는 형식은 안내와 함께 열기·내려받기를 제공해요. 불러오는 동안에는 자리를 지키는 **스켈레톤**이 뜨고, 목록에서 이미 받아둔 이미지는 스켈레톤 없이 곧바로 크게 보입니다. 본문에 넣은 이미지도 같은 스켈레톤이 적용되고 눌러서 크게 볼 수 있어요.
