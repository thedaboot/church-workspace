...
- 첨부 파일(클라우드 모드) — 수정 모드에서 드래그앤드롭·붙여넣기로 올립니다. 한 파일 25MB,
  ...
  앱 안 미리보기는 **전부 앱이 직접 그립니다** — PDF(pdf.js) · 이미지 · 영상 · 소리 · 텍스트에
  더해 **엑셀·워드·PPT도 우리 파서**로 읽습니다(`services/xlsx.js`·`docx.js`·`pptx.js`).
  올린 그 순간부터 보이고, 다크 모드와 톤이 맞습니다.
  엑셀은 원본 서식을 최대한 그대로 옮깁니다 — 한 셀 안에서 **구간마다 다른 색·굵기·크기**,
  칠·테두리(병합 상자 포함)·글자 크기·행 높이·열 너비, 숫자 서식(₩ 접두 · 회계식 0은 `-` ·
  음수 빨강·괄호 · 한국어 날짜 `2월 1일`과 요일), 조건부 서식(색눈금·데이터 막대·아이콘·수식),
  시트에 얹힌 **그림과 도형**, **틀 고정**(행·열)과 자동 필터 표시까지.
  가독성이 서식보다 앞서는 자리는 셋입니다(`SheetView.jsx` 머리 주석) — 넘치는 글자를 접고,
  열 너비에 상한을 두고, 글자를 엑셀보다 키웁니다.
