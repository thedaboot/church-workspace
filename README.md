...
- 댓글·@멘션 자동완성, 활동 기록(생성·상태·필드 변경·댓글·첨부).
  ...
  댓글을 다시 읽지 않습니다).
- 댓글 반응 — 댓글마다 **하트·좋아요·확인** 세 가지를 달 수 있습니다(다시 누르면 취소,
  한 사람이 셋 다 눌러도 됩니다). 숫자를 누르면 누가 눌렀는지 얼굴·이름으로 보입니다.
...
- 지금 누가 어디를 — 프로젝트 탭과 보드 카드에, 지금 그것을 보고 있는 사람 얼굴이
  최대 3명 겹쳐 뜹니다(본인 제외). 실시간 표시일 뿐 **어디에도 기록되지 않습니다** —
  그 사람이 나가면 얼굴도 같이 사라집니다.
