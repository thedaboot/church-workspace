...
- 읽기 실패에 '다시 시도' 자리(D2 — 예배·모임·멤버는 지금 토스트만) · 모바일 대시보드가 상태 칩을 숨기는 것(`dashboardParts.jsx:314` — 색만으로 상태를 말한다).
- 시각 통일(D9): 주 버튼 17벌 · 모달 틀 11벌 · 섹션 머리줄 6벌 · 인라인 저장/취소 순서 · 9px 이하 글자·faint 대비 · `Skeleton`에 넘긴 4·5px 모서리 여섯 곳이 지금 8px로 그려진다(PITFALLS 9-bw).
- **검색 결과 화면**(성경·업무 벡터) — 뒷단은 있다(아래 '다음'). 화면은 목업으로 묻는다.
...
2. **성경 검색은 AI 검색이 앞이다**(§7 — 벡터가 못 이겼다). 벡터는 AI 검색이 실패할 때의 대체 · 후보를 추려 AI에 넘기는 길의 후보다.
3. 검색 결과 화면(성경 대체 · 업무·댓글·첨부 통합 검색) — **목업 먼저**(★).
...
- **검색은 아직 `String.includes()`** 다(공백을 지우고 NFC로 비교 · 첨부 이름·댓글까지 · **내용은 못 본다**). RAG를 붙일 자리는 **첨부 내용·댓글·지난 프로젝트 아카이브**다 — **카드에 벡터를 붙이는 것부터 시작하지 마세요.** 벡터 뒷단은 0074(`doc_vec`)까지 있다 — 화면은 아직.
...
src/services/bibleSearch.js   AI 본문 검색 순수 모듈 — 참조만 받는다(§6-9-ar) · 한 번에 30줄(AI_HIT_LIMIT) · 벡터(0073)는 아직 화면에 없다
