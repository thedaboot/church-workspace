...
- **모바일 서식 바가 키보드 위에 서는지**(아이폰 사파리·카카오톡 인앱 · 안드로이드 크롬 · 업무 창·예배 노트·묵상 노트 셋 다) — 2026-09-11에 `bottom` 대신 **`top`으로 놓고 바를 `body`로 뺐다**(§6-9-aa-4).
  게스트 스위트는 키보드가 없어 **가짜 `visualViewport`로 흉내만** 낸다(`tests/handoff` 매트릭스) — 실제 키보드 높이·애니메이션은 실기기에서 봐야 한다.
