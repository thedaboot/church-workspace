...
- **홈 예배 카드는 그 주보 상세로 곧장 간다**(지난 예배도 같다) — 목록으로 보내고 다시 찾게 하지 않는다(사용자 결정 2026-09-18). 길은 **알림 딥링크와 같은 것**이다(`App.handleOpenLink` → `entryQuery` → `worshipView`의 진입 이펙트가 `?s=<주보 id>`를 받아 연다) — 주보 상세로 드는 문이 둘이 되지 않게, 홈은 `/?p=worship&s=…`를 넘기기만 한다.
