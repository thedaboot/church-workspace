...
- **업무·첨부**: 업무 만들기/삭제 · 댓글 쓰기/고치기/지우기 · 첨부 올리기(3MB 넘는 것 = Storage 경유 · **맥에서 고른 한글 이름 파일**이 멀쩡하고 검색에 걸리는지) · 본문 이미지·프로필 사진 · '구글 문서에서 편집' · 업무/프로젝트 삭제 때 폴더 휴지통.
- **미리보기·에디터**: 한글 PDF · **옛 iOS(18.0~18.3) 사파리에서 PDF** · 워드·PPT · HTML 첨부가 `frame-ancestors` 아래에서도 그려지는지 · 구글 iframe·유튜브·오피스 뷰어 · 에디터 서식 · 주보 PDF 내려받기(jspdf 4).
- **AI·푸시**: 요약·다듬기·순모임 가이드·성경 AI 검색 · 사진이 많은 업무의 요약이 문서 내용을 반영하는지 · 유튜브 재생목록 · 푸시 도착과 딥링크 · 크론 둘이 다음 날 로그에서 200인지(401이면 `CRON_SECRET`).
- **실시간·화면**: 홈 카드 실시간 갱신 · 출석 숫자(최근 8주) · 다크/라이트 전환 · 바깥 누름 닫기(모임 피커·검색·프로젝트 더보기).
...
- 시각 판단(2026-09-09 점검): `DepGraph` 고정 높이 440 · 768~900px 상단 검색창 · 375 주보 머리 카드 두 줄 · 1440 순원 줄의 '순 옮기기·빼기'가 이름에서 200px(§6-9-k) · '팀별 남은 업무'가 0건 팀까지 · 비밀번호 설정 줄 두 벌(`LinkPwFields`·`PasswordSetter`).

**다음 — 임베딩 뒷단(E)**(사용자가 이미 승인한 순서):

1. 0073: pgvector + `bible_vec`(`halfvec(768)`, **인덱스 없이** — 31,103절은 전수 비교로 충분하고 무료 500MB에서 인덱스가 데이터만큼 더 먹는다)
2. `scripts/embed-bible.mjs` — 성경 전체 색인은 **로컬에서 한 번만**(크론은 함수 시간 제한을 넘긴다). gemini-embedding-001, 약 $0.3 1회
3. 기존 AI 검색과 나란히 돌려 질의 30개로 품질 비교 → 이겨야 폴백으로 내린다
4. 통합 검색(카드 83 · 댓글 77 · 첨부 발췌 256 · 노트) 확장 — 증분은 **본문 해시**(updated_at 아님), 삭제는 FK on delete cascade
5. 크론은 **기존 8시 크론에 얹는다**(Vercel Hobby는 크론 2개 상한, 이미 2개 · `?job=embed`는 손으로 부르는 길) — 알림이 안 죽게 try/catch로 가른다. 검색 결과 화면은 품질을 잰 뒤 목업으로
...
- **검색은 아직 `String.includes()`** 다(공백을 지우고 NFC로 비교 · 첨부 이름·댓글까지 · **내용은 못 본다**). RAG를 붙일 자리는 **첨부 내용·댓글·지난 프로젝트 아카이브**다 — **카드에 벡터를 붙이는 것부터 시작하지 마세요.**
...
- **마이그레이션 번호별 표는 `README.md`에 하나만 둔다.** 스키마는 `supabase/migrations/0001~0072`이고 **전부 라이브 DB에 적용**되어 있다. 최근 것: **0063** 0061이 남긴 나머지 `auth.uid()` 자리를 `alter policy`로(§6-34-i) ·
  ...
  **0072** `files.name`을 NFC로(데이터만 · 되돌릴 수 없고 되돌릴 까닭도 없다). 다음 번호는 0073(§2).
...
- **RLS 경계 요약**: 모든 내용 표에 `is_approved()`(0022) · 관리자 원본은 `admins` 표 하나 · 지정·해제는 `is_master()`(0029) · 프로젝트 삭제는 전원(0021) · 주보 쓰기
  ...
  **조용히 옛 값으로 되돌린다** · §6-31-h) · storage 올리기·읽기도 `is_approved()`(0071). 자세한 것은 §4.5·§6-31-a.
