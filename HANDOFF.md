...
- **노트 도막마다 쓸 자리가 있다**(예배 노트·묵상 노트 둘 다). 그전에는 도막의 빈 줄이 한 줄(34px)이라 마지막 도막('기도')이 선에 딱 붙어 "적을 자리가 없는 칸"으로 보였다(사용자 신고). **재 보면 두 도막이 같은 높이였다** — 한쪽이 좁은 게 아니라 둘 다 한 줄이고 남는 자리가 전부 종이 맨 아래 한 덩어리에 몰려 있던 것이다. 그 자리를 도막마다 나눠 줬다(`index.css`의 `:is(h1,h2,h3,h4) + *` min-height). **읽기 종이에는 걸지 않는다** — 저장된 노트는 글 길이만큼만 서는 문서다.
