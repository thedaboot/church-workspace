...
- 0015: `cards.subtasks`(jsonb 체크리스트, 배열 제약 있음) + `cards.ai_summary`/`_at`/`_by`
  ...
  **관리자 제한은 화면에서 버튼을 감추는 것뿐입니다**(`canPin = cloudMode && isAdmin && id`).
  ...
  열려 있고, `isAdmin`도 클라이언트가 이메일을 비교해 판정합니다(`auth.jsx`). 요약은 다시
  만들면 되는 값이라 이대로 두기로 했습니다 — 진짜로 사라지는 일(프로젝트 삭제)은
  `is_admin()`으로 DB에서 막고 있습니다. 막으려면 요약 세 칸만 쓰는 security definer
  함수를 두고 그 안에서 `is_admin()`을 검사하는 길입니다(컬럼별 RLS 정책은 없습니다).
  `ai_summary_by`는 DB에만 남기고 **화면에는 이름을 붙이지 않습니다** — 읽는 사람이 그걸로
  할 일이 없고, 요약 내용보다 누가 손댔는지를 먼저 보게 됩니다. 같은 이유로 '고정' 배지도
  **관리자에게만** 보이고, 고정된 요약을 펼칠 때 **일부러 2초를 기다립니다**(`REVEAL_MS`).
  값이 이미 있어서 즉시 띄울 수 있는데도 그러는 이유: 즉시 뜨면 같은 버튼이 사람마다 다르게
  동작하고, 읽는 사람이 "누가 골라둔 글"임을 알아차립니다. 성능 문제로 오해하고 지우지
  마세요 — 사용자가 이 동작을 원해서 넣은 지연입니다.
