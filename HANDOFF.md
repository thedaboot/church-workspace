...
1. **폰·패드 화면 아래 절반이 간헐적으로 잘린다**(데스크톱도 가능성). 출발점: 앱 뿌리 높이가 `--app-vh`
   = `visualViewport.height`다(`App.jsx` 575~610 · 751). 가설 ① 손가락 확대(scale>1)나 키보드가 올라온 순간의 작은 값이
   남는다 ② 앱이 뒤로 갔다 돌아올 때(visibilitychange·pageshow·회전) resize가 안 와서 옛 값이 남는다. **재현부터**
   (§6-29-z-13·14 확대 함정 · PITFALLS 42-g). 실기기 확인 필수.
2. **갤럭시에서 푸시는 오는데 진동이 없다.** `public/sw.js`의 `showNotification`에 `vibrate`·`renotify`가 없고,
   `tag` 기본값이 `'thedaboot'` 하나라 **같은 tag의 두 번째 알림은 조용히 갈아 끼워진다**(renotify 없으면 소리·진동 없음)
   — 가장 먼저 볼 가설. `api/push.js`가 넣는 tag도 같이 볼 것. 안드로이드 크롬은 채널 설정(사이트 알림 → 소리/진동)도 영향.
3. **프로젝트 달력 ↔ 폰 기본 달력 연동**(추후). 13차 `api/ics.js`(서명 주소 · `SUPABASE_SECRET_KEY`에서 갈라 낸 HMAC)를
   딛고 **프로젝트별 구독 주소**(webcal · 마감이 있는 업무 → 일정)로 가는 게 자연스럽다. 목업·결정 먼저.
4. **조준환 청년에게 주보 수정 권한.** `can_edit_service()` = 관리자 · 올해 회장 · 교역자 · 미디어팀(0042·0045 · 라이브 확인).
   조준환은 명단 `teams {찬양팀, 순원}` · 2026 직분 `lead_team`. **사람 하나를 넣을지(명단 칸·예외 목록) 규칙으로 넣을지(예: 찬양팀장)**
   사용자에게 먼저 묻는다. 클라이언트 판정 `worship.js` 324~341의 `canEdit`도 같이 바꾼다(둘이 어긋나면 버튼만 서고 저장이 막힌다).
5. **대시보드·내 업무·팀 업무에서 상시를 보이지 않게.** 13차에 목록에 '상시' 구간을 따로 세웠다(`dashboardParts.jsx:52` ·
   셈은 `services/taskCounts.js`). 보드의 상시 줄은 그대로. 검사: `dashfix`·`traces`·`logcheck`의 상시 단정.
...
- **계정·권한(0071)**: 로그인 · 내 정보 저장(이름·사진·팀) — **합친 계정도**(0071 전에는 실패했다) · 관리자 승인/환송 · 계정 합치기 · 새 가입자가 승인 전에 못 들어오는지 · 명단 추가(순장 계정) · 알림 벨의 보낸 사람 이름.
  **승인 대기가 저절로 넘어가는지**(관리자가 수락하면 새로고침 없이 · 폰을 잠갔다 켜도) · 수락된 사람의 종에 '가입이 승인되었어요'.
...
src/services/domain.js        TaskService·ActivityService(§6-28) · 상시로 저장하면 날짜 두 칸을 지운다
src/services/taskCounts.js    업무 셈 한 벌 — 지연·마감 구간·남은 업무·2주 방치·진척·팀별/청년별(§6-28-c) — logcheck가 검사
...
| ↳ 성경 장 보기는 예외 | **사용자 결정 2026-09-25, 공동체성** — '이번 주 이 장을 본 사람'(0080 `bible_reads`: 누가·어느 장·어느 주만 · 나 제외 이름순 · 시각·횟수·절 없음 · 기본 켬, 판·내 정보의 '나도 나누기'로 끄면 지운다). 되돌리지 말 것 |
...
- **읽기 실패 자리**(D2): 캐시 없는 첫 읽기가 실패하면 빈 자리에 같은 그림 + 그 화면 토스트의 첫 줄 + `errorReason` + '다시 시도'(작은 단)를 세우고 토스트는 띄우지 않는다.
  캐시가 있으면 지난 값을 둔 채 토스트다. '없음'과 '못 읽음'을 상태로 가른다(빈 배열을 앉히지 않는다 · PITFALLS 24-g). 홈은 해당 없음. 동아리 모임·순모임 가이드도 이 자리다(2026-09-25).
- **가이드 고정 알림**(0077 · 사용자 문구 `이번 예배 순모임 가이드가 도착했어요!`): 그 해 순장에게 · **가이드당 한 번**(`pin_notified_at` — 풀었다 다시 걸어도 두 번 안 간다) ·
  누르면 `?p=groups&guide=<주보>`. 노트 공유 알림은 `note=<주보>`(PITFALLS 24-c-1).
- **동아리 모임 전날 알림**(0078 · 사용자 결정 2026-09-25): 모임 **전날 11:30**(`?job=worship` 배치의 두 번째 갈래 — 크론 자리가 둘뿐이고 8시는 이르다) 그 동아리 구성원+동아리장(만든 사람 포함)에게 `내일 {동아리 이름} 모임이 있어요` · preview는 모임 제목 · 누르면 `?p=groups&g=<동아리>` · 당일 알림은 없다. 동아리 이름은 **actor_name 칸**에 싣는다(시스템 알림이라 비어 있던 칸).
- **성경 낱말 검색**은 띄어쓰기를 지우고 견주고(`word.matchRanges`) 66권을 끝까지 센 뒤 50줄씩 '더 보기'로 편다. 결과에서 연 절의 되돌아가기는 `결과`다(목차가 아니다).
...
- **상시**(0075 · 2026-09-25 목업 C안): 마감 없이 계속 쓰는 업무(순번표·현황표·양식·메뉴얼) — 보드 네 칸 **위 한 줄**의 칩(보라 `상시 N` · 0건이면 줄 없음), 상태 고르기의 맨 아래.
  상시로 바꾸면 시작일·마감일을 지운다. 달력·지연·마감 구간·2주 방치·남은 업무·진척 분모에 들어가지 않고, 목록에서는 '상시' 구간에 따로 선다.
- **셈 기준**(2026-09-25 감사 · `services/taskCounts.js`): 한 주는 **주일~토요일**('이번 주' = 내일~토요일, 오늘은 '오늘 마감') · 날짜는 로컬 · **지연은 시작 전·진행 중만**(보류 중은 회색 날짜 · 제 구간) ·
  2주 방치는 **마지막으로 고친 날** 기준 · 대시보드 '팀별·청년별 남은 업무'와 내 업무·팀 보드의 프로젝트 칸은 **고른 해의 보관 안 한 프로젝트**만(KPI·마감 목록은 전부) · 청년별에 사람마다 지연 수를 두지 않는다.
...
- **실패 문구에 숫자·기술 용어를 넣지 마세요** — `드라이브가 50초 안에 응답하지 않았어요`를 `드라이브에 파일을 올리는 데 문제가 있어요 / 개발자에게 알려주시고, 잠시 뒤 다시 시도해주세요`로 고쳤다(몇 초 걸렸는지는 서버 로그에 남는다). 예외는 그림
  내보내기의 전면 실패 화면이다(§6-32-n). **원문(`e.message`·`String(e)`·브라우저 오류 이름·제품명)을 문구에 직접 넣지 않는다** — 뒷도막은 `errorText`(`failText`·`errorReason`)가 만들고
  원문은 콘솔에 남긴다(2026-09-25에 계정 연결 토스트·오류 화면·미리보기·공유 거부를 고쳤다). `errorReason`의 '아는 게 없으면 원문이라도'는 그대로다(§7 드라이브 502).
...
- **내 예배 노트 모아 보기**(예배 머리줄 '새 주보' 왼쪽 `내 예배 노트`): 쓴 것만 최근순 · `순에 공유` 칩 · 데스크톱 목록|종이 · 폰 목록→종이 · 종이 위 `주보에서 열기`. **0건 문구 `예배 노트가 아직 없어요`는 사용자가 직접 고른 것이다**
  (아래 문구 톤의 '없어요' 규칙보다 앞선다) + 노트 그림 + `{M월 D일} 예배 노트 쓰기`(발행본 중 오늘(KST)까지 드린 가장 최근 주보 → 그 상세의 노트 칸을 편집 상태로). 노트가 한 건이라도 있으면 그 버튼은 없다.
  `service_notes` 읽기 정책은 같은 순의 공유 노트도 주므로 **`profile_id`로 반드시 거른다**(`worship.fetchMyNotes`).
- **광고 → 내 달력**(방식 가 · `services/noticeDate.js` · `api/ics.js`): 광고 글에서 날짜가 읽힌 광고의 본문 아래에만 칩(`10월 11일 (일) 오후 2:00`). 주보 날짜 뒤만 · '생일' 줄 건너뜀 · 달만·연도만·'다음 주' 같은 상대 말은 안 읽는다
  (라이브 광고는 지금 하나도 안 읽힌다 — 정상). 넣는 길: 안드로이드 구글 캘린더 템플릿 · 아이폰/데스크톱 `.ics`(로그인 POST → **10분짜리 서명 주소** → 브라우저가 연다 — 토큰을 주소에 싣지 않는다) ·
  카카오 인앱은 `kakaotalk://web/openExternal`로 기본 브라우저에 넘김(안내 문구 없음) · 게스트 blob. **카카오 인앱 두 기기(아이폰·안드로이드)는 실기기 확인 대상이다.**
- **흔적과 움직임**(사용자 결정 2026-09-25 · 목업 `mockup-traces` 3~7 · `front-nudge` N1 · `mockup-grace` 4 — 권장안 그대로):
  앞 칸 넛지는 헷갈리는 순간에만 말풍선 하나(2.5초 · 구분선 accent) — 문구 `최근 활발한 프로젝트` / `최근 7일 동안 N명이 보고 있어요` · 뒤쪽 탭을 앞으로 끌 때 `앞에 있는 프로젝트는 자동으로 조정돼요.` + `이 프로젝트는 구분선 뒤에서 움직일 수 있어요.`
  (데스크톱 hover는 한 번 본 브라우저에서 안 뜬다 · PITFALLS 12-g·12-h) · 내 업무·팀 보드 기본 목록 맨 아래 `완료한 업무` + 흐린 `최근 7일`(오늘 포함 7일 · 로컬 날짜 · `taskCounts.isRecentlyDone`) ·
  최근 활동은 다섯 줄 → `더보기`(붙여 씀)로 열 줄씩 → `접기`, 주보 발행·동아리 모임 일정·나눈 QT 묵상(`OOO님이 오늘 QT 묵상을 나눴어요`)을 같은 줄 모양으로 시간순에 섞는다(`services/feedExtras.js` · 0079) ·
  지난 방문 이후 **남이** 움직인 활동 줄·프로젝트 탭 왼쪽에 5px accent 60% 점(숫자·글자 없음 · 탭 점은 열면 지움 · 폰 하단 '업무'에는 없음 · 기준 시각 `services/sinceSeen.js`) ·
  완료 손맛은 **두 자리만**(하위 업무 체크 · 마감 목록 완료 원 — 방금 누른 것만 · 되돌리기 즉시 · PITFALLS 9-cg·9-ch) · 동아리 상세 `다가오는 모임` / `지난 모임`(최근 셋 + `N건 더 보기`) · 비면 `다음 모임 미정` ·
  멤버 관리에서 수락하면 줄이 자리를 지키고 `수락됨` + `청년 명단과 잇기` 판(이름순 · 누르면 바로 연결 · 미리 고르지 않음), 미룬 사람은 `명단 미연결` 칩.
  순수 규칙은 `services/traces.js` 한 벌(logcheck) · 화면 검사는 `tests/traces`(+ `groups`·`roster`).
- **은혜와 리듬 묶음**(사용자 결정 2026-09-25 · 목업 `mockup-grace` 3·5·6·7·8 · `mockup-traces` 2 · 함정 PITFALLS 53-a~g):
  **오늘의 예배** — 발행된 주보 날짜가 오늘(KST)이면 08:00~자정(08시 뒤 발행이면 읽는 순간부터) 예배 카드가 격자 맨 앞 두 칸 · 말씀·찬양·광고·출석을 한 장에 · 13:30 전 찬양이 앞, 뒤 출석이 앞 ·
  내 순 카드는 출석 칸으로 들어가 빠진다 · 히어로 캐릭터는 그대로 · 광고는 주보 종이와 같은 모양 · 찬양 칸 머리에 `찬양 · 인도 OOO 형제`(2026-09-25 사용자 결정 — 오늘의 예배 카드에서만, 평소 카드는 2026-09-06대로 싣지 않는다 · 목업에는 있었다, ★ 다시 물을 것) · 판정 `services/homeMoments.js`.
  **지난 해의 오늘** — 태그라인 아래 알약 `작년 이맘때` + 작년 오늘 ±7일 activity가 가장 많은 작년 프로젝트(같으면 최근 · **모두에게 같은 줄**) · 누르면 그 창의 마감·완료 업무 셋 · 첫 줄은 2027-07-18 무렵.
  **발자취** — 12월 둘째 주일 ~ 1월 6일 같은 자리에 `{해}년의 발자취` 입구 → 홈 안의 한 장(App 전역 화면 아님 · '홈으로') · 부제 `예수님과 함께 걸어온 한 해` · 구역 `마음에 남긴 구절`·`다시금 펼치게 된 말씀`·
  `예배 노트를 작성한 주일`·`더다붓과 함께한 프로젝트` · 숫자·합계·순위·공유 없음 · 노트·묵상 글 없음 · 내 정보에서 다시 열기 없음.
  **QT 그 달 묵상** — 내 기록 카드의 달력 아래 가는 선 + 최근 날짜부터 '날짜 · 제목'(비면 구절 흐리게) · 7줄 넘으면 `N건 더 보기` · 누르면 그 날.
  **이런 마음일 때** — 빈 검색칸의 판 맨 위 칩 **한 줄**(폰 넷까지 · 실제 폭을 재서 넘치는 칩은 건너뜀) · 후보 50은 `src/data/moods.js`(칩 글자만) · AI 물음은 한 틀 `'{칩} 읽을 성경 말씀'` · 최근 검색어엔 칩 글자 ·
  닫고 1분 뒤 다시 열면 새로 섞고 방금 본 칩은 뒤로 · 게스트엔 칩 없음 · 칩이 있는 판은 검색 줄 전체 폭.
  **이번 주 이 장을 본 사람**(0080 `bible_reads`) — 장 머리 제목과 북마크 사이 얼굴 셋 + `+N` · 나 제외 이름순 · 판 제목 `이번 주 이 장을 본 사람` · 이 장에 걸친 이번 주 QT 묵상을 공유해 둔 사람만 `나눔 보기`(QT의 그 날·그 사람 종이로) ·
  판 아래·내 정보에 `나도 나누기`(기본 켬 · `bible_state.share_reads` · 끄면 내 줄을 지운다) · 장을 5초 넘게 펼쳤을 때만 남긴다 · 지난주 줄은 11:30 배치 끝이 지운다 · §7 '카드별 조회 추적'의 예외(그 행 아래).
