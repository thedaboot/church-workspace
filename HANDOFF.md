...
- **홈 예배 카드는 발행본만 세운다** — `status='published'` 중 `service_date`가 가장 최근인 것(미래 포함)이고, 머리 글자는 날짜가 정한다(`지난 예배` / `이번 주 예배`(오늘이 든 월~일) / `다음 주 예배`). **시한은 두지 않는다** — 몇 주 전 주보라도 그 자리에 선다(카드가 사라지는 것보다 낫다). 작성 중인 주보는 이제 홈에 안 오르므로 `작성 중 ·` 표시는 걷었다.
