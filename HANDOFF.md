...
- **폰에서 넘어간 것을 하단바가 말한다** — '업무'를 누르면 남색(`--app-night`)이 **오른쪽에서 왼쪽으로** 바를 덮고, '홈'을 누르면 **왼쪽에서 오른쪽으로** 원래 색이 돌아온다. 목업 넷(진한 accent · 딥 인디고 · 연한 파랑 · 윗선 띠)을 보고 사용자가 **딥 인디고**를 골랐다. **accent를 쓰지 않는 것이 핵심이다** — 진한 accent는 §8에서 '확정'이라는 뜻이라 상시 서 있는 바에 쓰면 화면마다 확정 버튼만 한 무게가 하나 더 생긴다. 배지 원은 그 층에서만 흰색이다. 짜임(층 둘을 겹치고 `clip-path` 왼쪽 끝 하나만 움직인다)은 `index.css`의 `.tab-bar-work` 주석에 있다.
...
- **키보드가 올라오면 레이아웃 뷰포트를 줄인다**(`index.html`의 `interactive-widget=resizes-content`). 노트 서식 바가 천장으로 가던 것이 그 때문이었다 — 바가 아니라 바가 붙어 있는 상자가 밀렸다. **실기기 확인이 필요하다**(아이폰이 이 값을 듣는지).
