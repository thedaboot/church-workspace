...
### 11차 손질 (2026-09-24) — 그릴링으로 정한 것

- **청년별 담당 업무에서 내린 줄은 아래 하위 업무에만 있다.** 위 구역에는 **안 내린 줄만** 남고, 다 내리면 구역이 사라진다. '하위 업무로'는 그 줄을 **본문 도막에서 지우고** `cards.subtasks`로 옮긴다 — 그 뒤로 담당자·기한의 기준은 subtasks 하나다(본문에 옛 이름이 남아 AI 요약이 읽지 않게). 다듬기를 다시 돌려 같은 제목이 또 뽑히면 `matchSubtask`로 거른다. 초록 '하위 업무' 표는 없앴다. 아래에서 지우면 그냥 지워진다(위로 안 돌아간다 · §7).
- **하위 업무 줄 = [체크][얼굴·이름][할 일] … [기한]**, 수정 모드에서는 **모든 카드**에서 위와 같은 담당자 칩·기한 칩으로 고친다(비면 점선 · 빈 칩 글자는 사용자 요청으로 `담당자`). 폰은 두 줄. 목업은 캔버스 6번(https://claude.ai/artifact/QaqMtHXiXAVeYevGBJyb7M).
- **하위 업무를 저장할 때 담당자·기한이 잘리고 있었다**(9-22부터) — `cardPatch`가 `{id,title,done}`만 적었다. `utils.subtasksForDb`로 고쳤다. **게스트 모드는 이 저장 길을 안 타서 브라우저 스위트가 못 봤다** — 클라우드 저장 모양을 바꾸는 기능은 `logcheck`에 단정을 둔다.
- 라이브 3장(10월 찬양 예배 변경 사항 및 피드백 · 10월 첫 주 찬양예배 기획 · 워크스페이스 11차 자체 개선)은 psql로 한 번 채웠다 — 제목으로 짝을 맞춰 빈 담당자·기한만 채우고 짝이 맞은 줄은 도막에서 걷었다. 위 저장 버그로 두 장이 한 번 더 잘려 되살렸다.
- **달력이 "위에서 오고 다른 데서 왔다"** — 재 보니 첫 프레임이 화면 왼쪽 위(6,7)에 그려지고 0.15초 동안 버튼 밑으로 미끄러졌다(PITFALLS 17-b). 앵커 팝오버 일곱 곳에 `transition-none`, 달력은 버튼 **왼쪽 끝**에 맞춰(`useAnchoredPos` `align:'start'`) 버튼 쪽 모서리에서 커진다. 잰 팝오버가 있으면 실제 폭으로 화면 안에 가둔다.

**실기기 확인**(이 손질 뒤 — 새로고침 후):

- 폰·데스크톱의 모든 날짜 칸에서 달력이 **버튼 바로 밑(모자라면 바로 위)에서 곧장** 뜨는지.
- 하위 업무를 체크해도 얼굴·기한이 **남는지**, 수정 모드의 담당자·기한 칩이 폰에서 두 줄로 들어오는지.
- 이전 세션에서 넘어온 것: 아이패드에서 한 시간 넘게 로그인이 남는지 · 다듬기로 할 일이 팀별로 다 뽑히는지 · 커서가 저장·취소 바 위에 서는지.

**다음 — 임베딩 뒷단**(사용자가 이미 승인한 순서 · 마이그레이션 0067~0070은 라이브 적용 끝, 다시 돌리지 말 것):

1. 0071: pgvector + `bible_vec`(`halfvec(768)`, **인덱스 없이** — 31,102행은 전수 비교로 충분하고 무료 500MB에서 인덱스가 데이터만큼 더 먹는다)
...
4. 통합 검색(카드 80 · 댓글 71 · 문서 20) 확장 — 증분은 **본문 해시**(updated_at 아님), 삭제는 FK on delete cascade
5. 크론은 `/api/push?job=embed`로 **기존 8시 크론에 얹는다**(Vercel Hobby는 크론 2개 상한, 이미 2개) — 알림이 안 죽게 try/catch로 가른다. 검색 결과 화면은 품질을 잰 뒤 목업으로

그 밖에 남은 것: 첨부 백필(문서 11건 → 사진 236건 Gemini Vision 캡션 · 승인됨) · 명단 미가입자 생일 3명(남다율·박세원·윤현서 — 값을 모른다) · 되돌리기 확인 ②③(ActionItems의 rows · 줄의 order — 메모리 부족으로 못 돌렸다).
