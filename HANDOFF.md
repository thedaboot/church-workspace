...
- **줄일 때 절 번호가 하나 밀렸다** — 코드 주석의 `§2-N`(흐름)은 지금 **§3**, `§3`(파일 지도)은 **§4**다. `§4.N`·`§6-N`은 `docs/PITFALLS.md`의 번호다.
...
- **Apps Script v10 배포 여부** — 큐시트 편집은 그것을 올린 뒤다(`docs/APPS_SCRIPT.md`의 '올린 뒤 확인할 것').

## 3. 이 레포의 흐름 (새 기능을 붙일 때)

기능을 여럿 붙이면서 매번 같은 순서를 밟았다. 그대로 하면 된다. (코드 주석의 `§2-N`이 아래 N단계다.)
