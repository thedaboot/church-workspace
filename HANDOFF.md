...
- **노트 도막마다 쓸 자리가 있다**(예배 노트·묵상 노트 둘 다). 그전에는 도막의 빈 줄이 한 줄(34px)이라 마지막 도막('기도')이 선에 딱 붙어 "적을 자리가 없는 칸"으로 보였다(사용자 신고). **재 보면 두 도막이 같은 높이였다** — 한쪽이 좁은 게 아니라 둘 다 한 줄이고 남는 자리가 전부 종이 맨 아래 한 덩어리에 몰려 있던 것이다. 그 자리를 도막마다 나눠 줬다(`index.css`). **빈 문단에만 건다** — 제목 다음 문단 전부에 걸었다가 같은 날 되돌렸다: 그 문단은 글을 쓰면 같이 자라서, 한 줄 쓰고 엔터를 치면 줄 간격이 세 줄처럼 벌어졌다(사용자 지적). **읽기 종이에도 걸지 않는다** — 저장된 노트는 글 길이만큼만 서는 문서다.
...
- **서식 바가 키보드에 밀리는 것은 아직 못 고쳤다.** `interactive-widget=resizes-content`를 넣어 봤다가 **아이폰이 안 들어서 되돌렸다**(사용자 스크린샷 2026-09-18). 바가 아니라 **바가 붙어 있는 상자(main)가 통째로 밀리는 것**이고, 이 화면은 홈 화면 앱(standalone PWA)으로 돈다 — 사파리와도 동작이 다르다. §6-9-aa-4대로 **실기기 `visualViewport` 값을 먼저 재고** 간다.
