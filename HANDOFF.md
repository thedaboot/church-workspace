...
- **폰에서 슬라이드 사본은 `/preview`**(드라이브식 미리보기)**, 데스크톱은 `/embed` 그대로**(화살표 · 2026-09-09 결정). `/embed` 발표 플레이어를 iframe으로 실으면 **홈 화면 앱 웹뷰가 통째로 죽었다**(사용자 재현 2회). 같은 주소를 앱 밖 사파리에서 열면 멀쩡하고, 같은 길로 가는 워드 첨부(`/preview`)는 폰에서도 잘 뜬다 — 무거운 것은 플레이어 하나다. **우리 렌더러(`services/pptx.js`)로 보내는 것은 사용자가 거부했다**("구글 화면으로 해달라니까" · 글자만 뽑아 그려 폰트가 깨진다). **실기기 확인이 필요하다** — `/preview`도 죽으면 남는 길은 새 탭뿐이다.
