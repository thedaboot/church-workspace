...
// 셋 · services/noteTemplate.js)으로 시작하면서 그 칸이 길어졌고,
...
// 도막 이름·순서(사용자 결정 2026-09-12 — '본문'을 뺐다. 바로 위에서 본 것처럼 구절은
// 종이 머리에 이미 서 있어서 도막으로 한 번 더 둘 자리가 아니다)
check('예배 노트는 세 도막으로 시작한다(구절은 머리에만)',
  notePaper.heads.join('|') === '말씀 요약|나의 결단|기도', JSON.stringify(notePaper.heads));
