...
| `navsmoke` `onebar` `mobbits` | 내비 구조, 데스크톱/모바일 내비가 하나만 마운트되는지, 모바일 상단바, **탭이 줄 폭만큼 보이는지**(1440px=8개 전부·800px=일부+더보기·무넘침), **더보기 연도 폴더**, **보관**(보관하면 그 탭에서 나가는지 · 열어 둔 보관 프로젝트가 탭과 더보기에 동시에 보이지 않는지) |
