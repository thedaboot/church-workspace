...
| 파일 | 보는 것 |
|---|---|
| `logcheck` `mdcheck` | 활동 기록 · 하위 업무 진척 · 인사말이 세는 범위(`utils.myScope`) · 마크다운 라운드트립 (브라우저 없이 순수 로직) |
...
| `three` | 프로젝트 이름 수정, **참고 링크**(포털 팝오버 안을 눌러도 닫히지 않는지 · 서비스 표시 · 저장), **헤더 배치**(375·414·768에서 삭제가 화면 안이고 링크 스크롤 칸 밖인지 · 모바일에서 공유·삭제가 오른쪽 끝이고, 삭제 아이콘 **자획**이 '새 업무' 오른쪽 선과 맞고, 공유 왼쪽 세로 실선이 '새 업무' 왼쪽 선과 맞고, 헤더 아래 실선 밖으로 나가는 것이 없는지 · '새 업무'가 값 줄과 같은 줄인지 · 진척 바 · 담당자 얼굴이 바 뒤인지 · 320px에서 값 줄이 넘치지 않는지), 모바일 가로 스크롤 잠금, 컬럼 구분 |
