...
| `three` | 프로젝트 이름 수정, **참고 링크**(포털 팝오버 안을 눌러도 닫히지 않는지 · 서비스 표시 · 저장), **헤더 배치**(375·414·768에서 삭제가 화면 안이고 링크 스크롤 칸 밖인지 · 모바일에서 공유·삭제가 오른쪽 끝이고 삭제 아이콘 자획이 '새 업무' 버튼과 같은 선인지 · '새 업무'가 값 줄과 같은 줄인지 · 진척 바 · 담당자 얼굴이 바 뒤인지), 모바일 가로 스크롤 잠금, 컬럼 구분 |
