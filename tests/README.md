...
| 파일 | 보는 것 |
|---|---|
| `logcheck` `mdcheck` | 활동 기록 · 하위 업무 진척 · 인사말이 세는 범위(`utils.myScope`) · 프로필 사진 주소 https 승격(`utils.httpsImage`) · 대시보드 사람 칸(다녀간·생일·합류·방문순·시간 단위 사다리) · 달력 생일 · 선후관계 열 배치(depLayers) · 선행 업무 활동 기록 · 마크다운 라운드트립 (브라우저 없이 순수 로직) |
