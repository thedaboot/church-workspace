...
export const AiService = {
  ...
  polishText: async (text, task = null) => {
    const prompt = [
      ...
      // 회의록에 할 일이 여럿이어도 한 줄만 뽑아 왔다 — 팀별로 흩어진 '남은 일'을
      // 하나도 빠뜨리지 않고, 이름이 없으면 팀으로 보내는 모양을 여기서 보여 준다.
      '---예시 4 (회의록 · 할 일이 여러 팀에 흩어져 있을 때)---',
      'before:',
      ...
      '',
      '### 청년별 담당 업무',
      '- @찬양팀 · 10월 콘티 확정 · 9월 26일까지',
      '- @웰컴팀 · Open Question 질문 컨펌하고 조편성 방법 정하기 · 9월 26일까지',
      '- @미디어팀 · 예배 전 영상 제작 현황 공유하고 업로드 마감일 정하기 · 9월 26일까지',
    ...
    const sysPrompt = [
      ...
      '- "정한 것"',
      '- "아직 정하지 못한 것"',
      '- "청년별 담당 업무" (이게 그대로 하위 업무 후보가 된다)',
      '  · 도막 제목은 **정확히 `### 청년별 담당 업무`**로 적어라. 다른 말로 바꾸지 마라.',
      '  · 이 도막의 줄은 **`- @누구 · 할 일 · N월 N일까지`** 모양으로 적어라.',
      '    화면이 이 줄을 읽어 하위 업무 후보로 세운다 — 모양이 흐트러지면 못 읽는다.',
      '  · **원문에 있는 할 일을 하나도 빠뜨리지 마라.** 이 도막은 회의록에서 가장 중요한 자리다.',
      '    "남은 일"·"~해주세요"·"~까지 공유"·"확인 요망"·"컨펌"·"마감"처럼 **누군가 움직여야 하는 것은 전부** 한 줄씩 세워라.',
      '    원문 곳곳에 흩어져 있어도 끌어모아라. 한 사람이 여러 개면 여러 줄로 적어라.',
      '    **여기서 요약하지 마라** — 위 도막들과 달리 이 도막은 빠짐없는 목록이어야 한다.',
      '  · 할 일은 **그 줄만 보고도 무엇을 하는지 알 수 있게** 구체적으로 적어라.',
      '    (나쁨: "확인", "피드백" / 좋음: "Open Question 질문 컨펌하고 조편성 방법 정하기")',
      '  · 맡는 쪽을 정하는 순서:',
      '    ① 원문이 사람 이름을 짚었으면 그 사람(`@조해리`).',
      '    ② 이름은 없고 팀만 짚었으면 **그 팀 이름**(`@찬양팀` · `@웰컴팀` · `@미디어팀` · `@엔지니어팀`).',
      '      "각 팀장님들은 …"처럼 여러 팀에 걸친 말이면 **팀마다 한 줄씩** 갈라 적어라.',
      '    ③ 일의 성격으로 팀이 분명하면 그 팀으로 보내라 — 콘티·송폼·찬양 곡은 찬양팀,',
      '      영상·PPT 디자인은 미디어팀, 송출·음향·장비는 엔지니어팀, 조편성·새가족·Open Question은 웰컴팀,',
      '      예산·일정 공지·전체 운영은 임원진이다.',
      '    ④ 그래도 모르겠으면 이름을 빼고 할 일만 적어라. **지어내지 마라.**',
      '  · 기한이 없으면 날짜를 빼라. "이번 주 내로"처럼 적혀 있으면 회의 날짜를 기준으로 그 주 일요일로 잡아라.',
      '',
