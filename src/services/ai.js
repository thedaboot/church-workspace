...
const ORG_CONTEXT = [
  '[우리 청년부]',
  '- 청년부는 약 55명이다. **워크스페이스 가입자가 청년부 전부가 아니다** — 여기 없는 사람이 더 많다.',
  ...
  // 사용자 결정 2026-09-25 — 두 자리를 섞어 불렀다(A/B에서 예배팀장을 '찬양팀장님'으로)
  '- **예배팀장은 찬양팀장과 다른 자리다.** 예배팀장은 찬양팀·엔지니어팀·워십팀에 걸친 예배 전체를 챙기고, 찬양팀장은 찬양팀을 이끈다.',
  ...
  // 직함 고르기는 **모델에게 맡기지 않는다**(2026-09-25) — "맥락에 맞는 쪽을 골라라"만 두었을 때
  // 순 일이 아닌데 순장님, 예배팀장을 찬양팀장님으로 불렀다. aiPeople.pickTitle이 골라 "부를 때"에 적는다.
  // 예시 이름은 지은 이름이다(실명을 쓰면 모델이 다른 카드에 그 사람을 끌어온다).
  '호칭 규칙:',
  '- "[이 업무에 관련된 사람]" 목록의 **"부를 때" 글자를 그대로** 써라(예: 한가람 총무님). 직함이 여럿인 사람은 이 업무에 맞는 직함을 이미 골라 두었다 — "순"·"맡은 일" 칸을 보고 직함을 바꾸지 마라.',
  '- 직함 뒤에는 언제나 "님"을 붙인다(총무님 · 부장님 · 전도사님). 교역자를 "교역자님"이라고 부르지 마라 — 목록에 적힌 직함으로 부른다.',
  '- "부를 때"가 "OOO 청년"인 사람은 그대로 "OOO 청년"이다. 직함을 지어 붙이지 마라.',
  '- 이름 옆 "(명단 이름 …)"은 같은 사람의 본명이다. 원문이 그 본명이나 이름 두 글자(예: 한가람 → 가람)로 적었어도 같은 사람으로 보고 "부를 때" 글자로 불러라.',
...
const CHURCH_CALENDAR_CONTEXT = [
  '[교회 일정의 기본 리듬]',
  '주일 4부 젊은이 예배 — 13:30 시작 · 3층 본당(은혜샘채플). 청년들은 "3층 본당"으로 부른다.',
  ...
  '달력 관련 규칙:',
  '- 날짜에는 요일이 괄호로 붙어 있다. 정기 예배와 겹치면 "주일 4부 젊은이 예배", "금요 열정 예배"로 불러라.',
...
// ── 명단 한 벌(순·연도 직분·명단 이름) ─────────────────────────────────────
// 스토어의 members에는 role_note·팀만 있다. 순 자리(TT순 순장)·연도 직분·표시명↔명단 이름
...
// 검사·A/B 도구가 명단을 직접 쥐여 주는 손잡이이기도 하다
export const setAiRoster = (value, at = Date.now()) => { rosterMemo = value ? { at, value } : null; };
const heldRoster = () => rosterMemo?.value || null;
async function loadAiRoster(now = Date.now()) {
  if (!supabase) return heldRoster();
  ...
  try {
    ...
    const value = await Promise.race([
      fetchRoster(year),
      new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('명단 조회 시간 초과')), ROSTER_WAIT_MS); }),
    ]);
    setAiRoster(value, now);
  } catch (e) {
    console.warn('[ai] 명단을 받지 못해 가입자 정보만으로 부른다:', e?.message || e);
  } finally {
    clearTimeout(timer);
  }
  return heldRoster();
...
// **글에 이름으로 나온 가입자**(2026-09-25 · 결정 13 — '### 이름 두 글자'·'OO순'·명단 이름처럼 @ 없이 적힌 사람.
// 찾는 규칙은 aiPeople.mentionedMembers). `text`를 주면(다듬기의 초안) 그 글에서 찾고,
// 안 주면 제목·본문에서 찾는다. 가입자만 싣는다 — 명단에만 있는 사람은 부를 멘션이 없다.
// 한 줄 모양·직함 고르기는 aiPeople.personLine·pickTitle이다(결정 1·3·9).
export function peopleContext(task, related, { withMention = false, text = null } = {}) {
  const members = store.getState().members || [];
  if (!members.length) return '';            // 게스트 모드에는 사람 목록이 없다
  const index = rosterIndex(heldRoster(), members);
  const names = new Set();
  const add = (arr) => (arr || []).forEach(n => n && names.add(n));
  add(task.assignees);
  (related || []).forEach(t => add(t.assignees));
  const said = [task.content || '', text || '', ...(task.comments || []).map(c => c?.text || '')].join('\n');
  extractMentions(said).forEach(n => names.add(n));
  const body = text ?? `${task.title || ''}\n${task.content || ''}`;
  mentionedMembers(body, members, index).forEach(n => names.add(n));
  const picked = members.filter(m => names.has(m.name));
  if (!picked.length) return '';
  const scope = taskScope(task, body);
  return [
    '[이 업무에 관련된 사람]',
    ...picked.map(m => personLine(m, { info: index.get(m.name), scope, withMention })),
  ].join('\n');
}
...
//
// **`업무:` 접두는 있어도 없어도 받는다.** 프롬프트는 `[[업무:제목]]`으로 쓰라고 하지만
// 실제 Gemini가 `[[5차 피드백 개선]]`처럼 접두를 빼고 썼고, 접두 필수 정규식은 그걸
// 통째로 무시해서 **본문에 `[[…]]`가 생짜로 남았다**(사용자 지적 2026-08-30 — 라이브).
// `[[…]]`는 우리 표시 문법이라 본문에 정당하게 나올 일이 없으니 전부 소비한다 —
// 제목이 맞으면 링크, 아니면 괄호를 벗겨 글자로. 어느 쪽이든 `[[`는 안 남는다.
...
export function buildTaskContext(task, now = new Date()) {
  ...
  // 남은 하위 업무가 **없을 때도** 챙길 것의 재료를 가리킨다(2026-09-25 · 결정 11). 예전에는
  // '없음(전부 완료)'이라고만 적거나(다 끝났을 때) 아무 줄도 없어서(안 나눴을 때), 모델이
  // "현재 남은 하위 업무는 없어요"를 챙길 것 자리에 그대로 썼다 — §8이 가장 싫어하는 모양이다.
  const subLine = subs.length
    ? [`- 하위 업무 ${subDone.length}/${subs.length} 완료`,
       subLeft.length
         ? `- 남은 하위 업무(챙길 것은 이것뿐이다): ${subLeft.slice(0, 8).join(', ')}`
         : '- 하위 업무는 모두 끝났다(챙길 것은 상세 내용·댓글에서 아직 안 끝난 일이나 마무리로 확인할 것을 골라라)',
       subDone.length ? `- 이미 끝낸 하위 업무(끝났다 — 챙기라고 말하지 마라): ${subDone.slice(0, 8).join(', ')}` : '']
        .filter(Boolean).join('\n')
    : '- 하위 업무는 아직 나누지 않았다(챙길 것은 상세 내용·댓글에서 아직 안 된 일을 골라라)';
...
// AI를 물어볼 수 있는 자리인가(= 로그인이 가능한 빌드인가). 게스트 모드에는
// supabase 클라이언트 자체가 없어 callGemini가 곧바로 안내 문구를 돌려준다 —
// **묻기 전에** 이걸로 갈라야 화면에 기다리는 자리(스켈레톤)가 한 번도 안 뜬다
// (본문 검색의 'AI가 찾은 구절' 칸이 그렇다 · components/wordBible.jsx).
...
export const AiService = {
  ...
  summarizeTask: async (task) => {
    ...
    const sysPrompt = [
      ...
      '- "챙길 것"에는 **"남은 하위 업무" 줄이 있으면 그 줄에 적힌 것만** 써라. "이미 끝낸 하위 업무" 줄의 항목을 챙기라거나 마무리하라고 쓰면 그 요약은 틀린 것이다.',
      '  남은 하위 업무가 없으면(안 나눴거나 다 끝났으면) **상세 내용과 댓글에서 아직 안 끝난 일**을 골라 써라.',
      '  본문에도 남은 일이 보이지 않으면 마무리로 확인할 것(공유 · 정리 · 인수인계)을 써라.',
      '  **하위 업무가 없다고만 적고 끝내지 마라.** 챙길 것 줄은 언제나 할 일 하나를 말한다.',
  ...
  polishText: async (text, task = null) => {
    ...
    const prompt = [
      ...
      // 예시 3도 사람 목록을 보여 준다 — 원문의 이름 두 글자('가람')를 목록의 사람('한가람 총무님')으로 잇는 것이
      // 이 예시가 가르치는 것이다.
      '---예시 3 (회의록 · 피드백 및 강평회)---',
      '이 예시에서 "[이 업무에 관련된 사람]"은 이렇게 주어졌다고 하자:',
      '  윤도하 | 팀: 임원진·찬양팀 | 부를 때: 윤도하 리더순장님 | 멘션은 @윤도하',
      '  오지안 | 팀: 임원진·찬양팀 | 부를 때: 오지안 예배팀장님 | 멘션은 @오지안',
      '  한가람 | 팀: 찬양팀·임원진 | 부를 때: 한가람 총무님 | 멘션은 @한가람',
      '  서다온 | 팀: 임원진·웰컴팀 | 부를 때: 서다온 리더팀장님 | 멘션은 @서다온',
      'before:',
      '8/24 강평회 함 참석 도하 지안 가람 다온 카운트다운 늦게 시작한거 지적나옴 다음주부터 25분에 딱 틀기로 함 송폼 오타 있었대서 가람이가 다시 보기로 조명 어두웠다는 얘기 있었는데 결론 못냄 다음 강평회때 다시',
      'after:',
      '### 8월 24일 피드백 및 강평회',
      '- 참석: 윤도하 리더순장님, 오지안 예배팀장님, 한가람 총무님, 서다온 리더팀장님',
      '',
      '### 정한 것',
      '- 카운트다운 영상을 ==13시 25분에 정확히 시작==하기로 했어요 (다음 주부터)',
      '- 송폼 오타는 @한가람 님이 다시 확인하기로 했어요',
      '',
      '### 아직 정하지 못한 것',
      '- 조명이 어두웠다는 의견이 있었고, ==다음 강평회에서 다시 다루기로== 했어요',
      '',
      '### 청년별 담당 업무',
      // 기한이 원문에 없으면 날짜를 붙이지 않는다 — 예전 예시는 '8월 31일까지'를 지어 붙였다
      '- @한가람 · 송폼 오타 다시 확인',
      // 카운트다운을 트는 것은 엔지니어팀이다(ORG_CONTEXT) — 목록에 그 팀 사람이 없으면 팀 이름으로
      '- @엔지니어팀 · 다음 주부터 카운트다운 영상을 13시 25분에 정확히 틀기',
      '---예시 끝---',
      '',
      ...
      // 그래서 예시의 달을 지금과 먼 4월로 옮기고, 기한이 없는 줄은 날짜 없이 둔다.
      '---예시 4 (할 일이 여러 팀에 흩어져 있을 때 · 사람 목록을 보고 내린다)---',
      '이 예시에서 "[이 업무에 관련된 사람]"은 이렇게 주어졌다고 하자(이 예시의 오늘은 4월 13일이다):',
      '  한가람 | 팀: 찬양팀·임원진 | 부를 때: 한가람 총무님 | 멘션은 @한가람',
      '  서다온 | 팀: 웰컴팀·미디어팀 | 부를 때: 서다온 리더팀장님 | 멘션은 @서다온',
      '  최서율 | 팀: 찬양팀 | 부를 때: 최서율 청년 | 멘션은 @최서율',
      'before:',
      '4월 찬양예배 주제 전도에서 예배자로 바꿈 예배시간 15시30분에서 15시로 당김 각 팀장님들 17일까지 댓글로 진행상황 공유 찬양팀 콘티 이번주내 확정 웰컴팀은 오픈퀘스천 질문 컨펌하고 조편성 방법 정하기 미디어팀 예배전영상 제작현황이랑 업로드 마감일 엔지니어팀 송출계획 역할분담 ppt 완료일정 4월25일 연습가능한지 확인필요',
      'after:',
      '### 4월 찬양 예배 준비',
      '',
      '### 정한 것',
      '- 4월 주제를 전도에서 ==예배자==로 바꿔요',
      '- 예배 시작을 **15시 30분**에서 **15시**로 당겨요',
      '',
      '### 아직 정하지 못한 것',
      '- ==4월 25일 연습이 가능한지== 팀원·팀장 일정을 확인해야 해요',
      '',
      '### 청년별 담당 업무',
      ...
      '- @서다온 · 예배 전 영상 제작 현황 공유하고 업로드 마감일 정하기 · 4월 17일까지',
      // 엔지니어팀은 목록에 아무도 없다 → 팀 이름으로
      '- @엔지니어팀 · 영상 송출 계획과 역할 분담 정하고 PPT 완료 일정 잡기 · 4월 17일까지',
      // 누구 몫인지 글에도 없고 팀도 안 갈린다 → 이름 없이 할 일만. 기한도 적힌 것이 없다
      '- 4월 25일 연습이 가능한지 팀원과 팀장 일정 확인',
      '---예시 끝---',
      '',
      task ? '\n' + buildTaskContext(task) : '',
      '',
    ...
    const sysPrompt = [
      ...
      '- "아직 정하지 못한 것"',
      '',
      // ── 청년별 담당 업무 도막 (2026-09-22 그릴링으로 정한 규칙) ──────────────
      // **회의록 블록 밖으로 꺼냈다.** 전에는 "회의록이면 이 구조로" 안에 있어서,
      // 기획안·아이디어 목록에서는 도막이 아예 안 만들어지거나 한 줄만 나왔다.
      '[청년별 담당 업무] — **이 도막은 회의록만의 것이 아니다.**',
      '- 글 안에 **누군가 움직여야 끝나는 것**이 하나라도 있으면 맨 아래에 이 도막을 만들어라.',
      '  하나도 없으면 도막 자체를 만들지 마라(빈 도막을 남기지 마라).',
      '- 도막 제목은 **정확히 `### 청년별 담당 업무`**다. 다른 말로 바꾸지 마라.',
      '- 줄 모양은 **`- @누구 · 할 일 · N월 N일까지`**다. 화면이 이 줄을 읽어 세운다 — 흐트러지면 못 읽는다.',
      '',
      '무엇을 세우나:',
      '- 회의록의 "남은 일"·"~해주세요"·"확인 요망"·"컨펌"은 물론이고,',
      '  **기획안의 항목 목록**(주요 개선 항목·주차별 흐름·예배 구성 등)도 누군가 손대야 하는 것이면 세워라.',
      '- **이미 끝난 사실과 정해진 결정은 세우지 마라** — "주제를 예배자로 바꿈"은 결정이지 할 일이 아니다.',
      '- 원문 곳곳에 흩어져 있어도 끌어모아라. 한 사람이 여러 개면 여러 줄로 적어라.',
      '- **여기서 요약하지 마라.** 위 도막들과 달리 이 도막은 빠짐없는 목록이어야 한다.',
      '- 할 일은 그 줄만 보고도 무엇을 하는지 알 수 있게 적어라.',
      '  (나쁨: "확인", "피드백", "세부 내용 수립" / 좋음: "Open Question 질문 컨펌하고 조편성 방법 정하기")',
      '',
      ...
      '  예산·전체 일정·팀 배분은 임원진.',
      '- 그래도 모르겠으면 이름을 빼고 할 일만 적어라.',
      '',
      '기한:',
      '- **적힌 것만 붙여라.** 없으면 날짜를 빼라 — 지어내지 마라.',
      '- 다만 "각 팀장님들은 N일까지"처럼 **여럿에 걸리는 기한**이 적혀 있으면 그 기한이 걸린 줄에만 나눠 붙여라. 다른 할 일로 번지게 하지 마라.',
      '- 달이 빠진 날짜("N일까지")는 오늘 기준으로 가장 가까운 앞날로 읽어라.',
      '- **예시에 나온 날짜를 옮겨 적지 마라.** 예시의 날짜는 예시 안에서만 뜻이 있다.',
      '',
      '우리가 쓰는 말로 바꿔라(다른 말로 바꾸지 마라):',
      '- 예배당·본당 → 3층 본당    미팅·회의 → 리더 모임(임원진 모임일 때)',
      '- 주일예배·4부 → 주일 4부 젊은이 예배    금요예배 → 금요 열정 예배',
      '- 셀·소그룹 → 순    조는 수련회 때만 쓰는 말이다',
      '- 곡 순서 → 콘티    악보 양식 → 송폼',
      '',
      '멘션:',
      '- 사람이 원문에 등장하면 "[이 업무에 관련된 사람]"에 적힌 **멘션 표기 그대로** @를 붙여 불러라.',
      '  표기가 이름과 다를 수 있다(예: 명단 이름이 한가람이어도 표시명이 가람이면 @가람). 적힌 대로 써라.',
