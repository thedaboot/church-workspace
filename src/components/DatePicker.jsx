    ...
    <div ref={popRef} data-datepicker="" role="dialog" aria-label={ariaLabel}
      ...
      className="z-[90] w-max bg-surface border border-line rounded-lg shadow-elevated p-3 transition-none animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center justify-between mb-2">
            <button type="button" onClick={goPrev} disabled={!canPrev} className={`relative before:absolute before:-inset-2 p-1 rounded-md text-fg-muted transition active:scale-95 ${canPrev ? 'hover:bg-surface-hover' : 'opacity-30 cursor-not-allowed'}`}><ChevronLeft size={16} /></button>
            ...
            <button type="button" onClick={goNext} disabled={!canNext} className={`relative before:absolute before:-inset-2 p-1 rounded-md text-fg-muted transition active:scale-95 ${canNext ? 'hover:bg-surface-hover' : 'opacity-30 cursor-not-allowed'}`}><ChevronRight size={16} /></button>
          </div>
          {!yearless && (
