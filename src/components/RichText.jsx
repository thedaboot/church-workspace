  ...
  return (
    <>
      {parsedBlocks.map(block => {
        if (block.type === 'image') return <div key={block.key} className="my-2"><img src={block.value} alt="embedded" className="max-w-full rounded-md border border-line max-h-64 object-contain" /></div>;
        if (block.type === 'line') return (
          <p key={block.key} className="mb-1 text-fg leading-relaxed text-sm">
            {block.tokens.map(t => {
              if (t.type === 'mention') return <span key={t.key} className="text-accent font-semibold bg-accent-weak px-1 rounded mx-0.5">{t.value}</span>;
              if (t.type === 'link') return <a key={t.key} href={t.value} target="_blank" rel="noreferrer" className="text-accent underline mx-0.5 break-all hover:text-accent-strong">{t.value}</a>;
