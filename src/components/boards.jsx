...
export const CalendarBoard = React.memo(({ tasks, onTaskClick }) => {
  ...
  return (
    <div className="bg-surface rounded-md border border-line flex flex-col h-full min-h-[400px]">
      <div className="px-4 py-3 border-b border-line bg-surface-2"><h3 className="font-bold text-sm text-fg">{currentYear}년 {currentMonth + 1}월</h3></div>
  ...
  return (
    <div className="flex gap-4 h-full pb-2 overflow-x-auto snap-x snap-mandatory">
      {CONFIG.STATUSES.map(status => (
        <div key={status} className="flex-1 min-w-[280px] max-w-[350px] flex flex-col bg-surface-2 rounded-md p-3 border border-line snap-center h-full" onDragOver={e => e.preventDefault()} onDrop={e => onDrop(e, status)}>
          <div className="flex items-center justify-between mb-3 px-1 shrink-0">
            <h3 className="font-semibold text-sm text-fg flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${status === '시작 전' ? 'bg-fg-faint' : status === '진행 중' ? 'bg-accent' : 'bg-tag-green'}`}></div>{status} <span className="text-fg-faint text-xs font-normal">{tasks.filter(t => t.status === status).length}</span></h3>
          </div>
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 pb-4">
            {tasks.filter(t => t.status === status).map(task => (
              <div key={task.id} draggable onDragStart={e => onDragStart(e, JSON.stringify(task))} onClick={() => onTaskClick(task)} className="bg-surface p-3.5 rounded-md border border-line cursor-pointer hover:border-accent hover:shadow-sm transition-all group">
            ...
            <div className="h-16 border-2 border-dashed border-transparent rounded-md flex items-center justify-center text-xs text-fg-faint opacity-0 hover:opacity-100 hover:border-line transition-all">여기로 드래그</div>
          </div>
        </div>
      ))}
    </div>
