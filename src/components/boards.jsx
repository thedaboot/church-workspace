  ...
  return (
    <div className="flex gap-4 h-full pb-2 overflow-x-auto snap-x snap-mandatory">
      {CONFIG.STATUSES.map(status => (
        <div key={status} className="flex-1 min-w-[280px] max-w-[350px] flex flex-col bg-surface rounded-lg p-3 border border-line snap-center h-full" onDragOver={e => e.preventDefault()} onDrop={e => onDrop(e, status)}>
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 pb-4">
            {tasks.filter(t => t.status === status).map(task => (
              <div key={task.id} draggable onDragStart={e => onDragStart(e, JSON.stringify(task))} onClick={() => onTaskClick(task)} className="bg-surface p-3.5 rounded-lg border border-line cursor-pointer hover:shadow-soft transition-all group">
