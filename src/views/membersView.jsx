...
//
// 세 가지만 한다. 승인 대기 수락 · 관리자 지정·해제 · 내보내기.
// '내보내기'는 접근만 끊는다(0022) — 지난 댓글·기록의 이름은 그대로 남는다.
// 계정을 지우는 길은 두지 않았다: 뭔가 쓴 적 있는 사람은 DB가 삭제를 막는다.
//
...
export function MembersView({ isAdmin }) {
  ...
  const approve = async (row, next) => {
    ...
    try {
      ...
      showToast(next ? `${row.display_name || '이 분'}을 수락했어요` : `${row.display_name || '이 분'}의 접근을 끊었어요`);
    } catch (e) {
      console.error('[cloud] 승인 변경 실패:', e);
      showToast(failText(next ? '수락하지 못했어요' : '내보내지 못했어요', e));
  ...
  const dropAdmin = async (email) => {
    ...
    try {
      ...
      showToast('관리자에서 내렸어요');
    } catch (e) {
      console.error('[cloud] 관리자 해제 실패:', e);
      showToast(failText('관리자에서 내리지 못했어요', e));
    } finally { mark(email, false); }
  };
  ...
  return (
    <div className="dc-screen max-w-3xl mx-auto pb-8">
      <div className="mb-6">
        <h2 className="text-lg md:text-xl font-extrabold text-fg tracking-[-0.4px]">멤버</h2>
      ...
      {rows === null ? (
        ...
        <>
          ...
          <Section title="함께하는 사람" count={members.length}>
            {members.map(row => (
              <MemberRow key={row.id} row={row} action={
                <ConfirmPopover message={`${row.display_name || '이 분'}의 접근을 끊을까요? 지난 댓글·기록은 그대로 남아요.`}
          ...
          <Section title="관리자" count={(admins || []).length}
            ...
            {(admins || []).map(email => (
              <div key={email} className="flex items-center gap-2.5 py-2.5" style={{ borderBottom: '1px solid var(--app-line)' }}>
                ...
                <ConfirmPopover message={`${email}을 관리자에서 내릴까요?`} onConfirm={() => dropAdmin(email)}>
