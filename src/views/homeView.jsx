...
// 종류는 가리지 않는다: 금요 예배만 발행돼 있으면 그것이 홈에 서고, 무슨 예배인지는
// kindLabel이 말한다(결정 14).
...
//   · 오늘보다 앞 → '지난 예배'
//   · 오늘        → '오늘 예배'
//   · 오늘보다 뒤 → '다가오는 예배'   (두 주 뒤라도 아직 안 온 예배다)
//
// **주를 보지 않는다**(사용자 결정 2026-09-18). 2026-09-14~18에는 '이번 주 / 다음 주'로
// 갈랐는데 주의 시작을 어디로 두든 한쪽이 틀렸다: 월요일 시작이면 금요일에 보는 다가오는
// 주일이 '이번 주 예배'가 되고(사용자 신고 — 9/18 금에 9/20 주보가 '이번 주 예배'),
// 주일 시작이면 월요일에 보는 어제 주일이 '이번 주 예배'로 남는다. 게다가 예배는 주일만이
// 아니라 금요 열정 예배도 있어서(§7), 주 단위 말은 금요 예배 주보에 애초에 안 맞는다.
// 오늘과 주보 날짜만 견주면 두 문제가 같이 없어지고 주 경계 셈이 통째로 빠진다.
// 무슨 예배인지는 바로 아래 메타 줄(kindLabel)이 말한다.
//
// 오늘은 언제나 한국 시간(kstToday)이고 여기서 새 시간대 셈을 만들지 않는다.
// **Date를 만들지 않는다** — `YYYY-MM-DD`는 자리 수가 고정이라 문자열 비교가 곧 날짜
// 비교다(pickService의 정렬과 같은 근거다). 시간대에 하루가 밀릴 자리가 아예 없어진다.
export function homeWorshipLabel(iso, today = kstToday()) {
  ...
  if (date < base) return '지난 예배';
  return date === base ? '오늘 예배' : '다가오는 예배';
}
...
// 따로 계산된다 — 행끼리는 달라도 된다). 그래서 오늘의 QT·예배 행은 두 줄
...
// ── 카드 자리 넷의 차례 ─────────────────────────────────────────────────────
// 순서는 docs/V2.md §3 그대로 고정이다 — 오늘의 QT · 예배 · 내 업무 · 내 순.
// 갈래마다 상태가 셋이다:
