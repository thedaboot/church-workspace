...
**34-g.** **합친 계정으로 '내 정보'를 고치면 자기 행에 써져 화면에 안 보인다** — 보이는 것은 남긴 계정 (`loadCloudState`의 `shownProfile`)인데 저장은 `auth.uid()`로 간다(`profiles_update`는 0001
    그대로). **사용자 결정 2026-09-10 — 고치지 않고 적어 둔다.** 고칠 때는 그 정책을 `effective_uid()`로 옮기고 `updateMyProfile`·`setMyTeams`를 `myUid()`로 바꾼다.
...
### 업무 창의 링크는 첨부 구역 안에 있습니다 (2026-09-10)

**35. 자리가 뜻을 말한다.** `+ 참고 링크`(0058)가 업무 창 본문 아래 혼자 서 있어 왜 있는지가 읽히지 않았다. 본뜻은 **첨부의 한 갈래**("구글 시트는 링크로도 첨부해서 그 시트는 편집까지 되게")이므로 기능은 그대로 두고 자리와 이름만 옮겼다 —
    '첨부 파일' 구역 안 **한 목록**, 버튼은 `+ 파일`·`+ 링크` 둘.

- **저장하는 길은 바뀌지 않았다**(`resource_links` · `modals.linkOps` + `SYNC_TASK` + `cloudSync.link*Cloud`) · 끌어다 놓는 자리는 이제 **구역 전체**다(붙여넣기는 예전 그대로 document
  리스너).
- **새 업무(id 없음)에는 `+ 링크`를 두지 않는다**(링크 행이 카드를 참조하고, 저장이 실패하면 적어 둔 링크가 사라진다).
- **지우기·비밀번호는 수정 모드에서만** — 한 목록에서 규칙이 갈리면 규칙이 없는 것으로 읽힌다.
- **게스트에서도 구역이 선다**(링크만) — 파일도 링크도 없고 달 수도 없으면 구역째 안 그린다.
- 프로젝트 헤더는 **그대로 칩**이고 버튼도 `+ 참고 링크`다(§4.6 · §8).
