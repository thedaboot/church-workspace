...
9-aa. **sticky의 통은 `overflow != visible`로 찾아야 한다** — `auto|scroll`만 보면 `overflow:hidden` 조상을 지나쳐 재는 상자와 붙는 상자가 달라진다(`MarkdownEditor.useStickyTop` ·
    ...
    있을 때만 서고(blur 뒤 180ms 기다렸다가 `activeElement`가 편집기 밖일 때만 내린다 — 바를 누르는 순간의 blur를 그래야 무시한다 · **바가 포털로 빠져 있으므로 감싸개와 바를 둘 다 본다**),
    자리는 키보드가 있으면 `top`(9-aa-4)이고 없으면 `--mobile-tab-bar-h`나 안전 영역 위의 `bottom`이다(**풀스크린 창 안에서는 탭바가 안 보이므로 안전 영역만** —
    조상에 `position: fixed`가 있는지로 가른다). 편집 칸은 포커스 중에만 바 높이만큼 아래를 비운다. **배경은 두 판 모두 불투명이고 블러는 없다.** 헤드리스는 창에 포커스가 없어 `focus()`가 이벤트를
    내지 않는다 — 검사에서는 `Emulation.setFocusEmulationEnabled`를 켜세요(`tests/handoff`·`tests/groups`). 폭 여섯 × 자리 셋 매트릭스가 `tests/handoff`에 있다(갈래를 가르는 것은
    `useIsMobile`의 `max-width: 767px` 하나 — 태블릿은 sticky 갈래이고, 거기서 키보드가 올라와도 바는 **위**에 서므로 상관이 없다).
9-aa-2. **편집기가 만들어질 때 이미 포커스를 물고 있을 수 있다** — 그때는 focus 이벤트가 우리 손에 들어오기 전이라 모바일 바가 서지 않았다. `editor.isFocused`를 한 번 읽어 상태를 맞춘다.
9-aa-3. **'붙었나'를 보는 관찰자의 `root`는 바가 붙는 그 통이어야 한다**(2026-09-11) — 비워 두면 기본이 뷰포트다. 업무 창은 통이 화면 한가운데라 우연히 맞았지만 **페이지 통**(App의 `main`)에서는
    센티넬이 통의 잘림선(화면 위 53px)에서 이미 사라져 `boundingClientRect.top < 0`이 영영 거짓이었다 — 바가 붙어도 모양이 상자 그대로였다. `useStickyTop`이 찾은 통을 `root`로 넘기고,
    가르는 기준은 0이 아니라 `entry.rootBounds.top`으로 본다. 깎는 양은 `-(통의 paddingTop + stickyTop)`이다(**sticky의 `top`은 통의 콘텐츠 상자에서 잰다** — 그래야 붙는 순간과 한 프레임도 안 어긋난다).
    **그리고 rootMargin의 아래는 통째로 열어 둔다**(`100000px`) — 관찰자는 '겹침이 바뀔 때'만 부르는데, 아래도 닫으면 편집기가 통 밑에 있을 때도 '안 겹침'이라 거기서 한 번에 위로 뛰면(닻 이동·빠른 튕김)
    안 겹침 → 안 겹침이 되어 **아무도 안 불린다**(조금씩 굴리면 붙고 0에서 1161로 뛰면 안 붙었다).
9-aa-4. **모바일 바는 `bottom`이 아니라 `top`으로 놓고, `body`로 빼낸다**(2026-09-11 · 실기기 스크린샷 둘). ① iOS 사파리의 `position: fixed`는 **레이아웃 뷰포트** 기준인데 키보드가 열려도
    `innerHeight`가 그대로일 수도 줄 수도 있어 `bottom = innerHeight - vv.height - vv.offsetTop`은 믿을 수 없다 — **`top = vv.offsetTop + vv.height - 바높이`·`bottom: auto`** 로 놓는다
    (`resize`·`scroll` 둘 다 듣고 `requestAnimationFrame`으로 한 프레임에 묶는다 · 가리는 것이 없으면 예전의 탭바/안전 영역 갈래). ② 그보다 먼저 **transform이 걸린 조상이 있으면 fixed의 기준이
    뷰포트가 아니다**(§6-1과 같은 함정) — 화면 뿌리 `.dc-screen`의 등장 애니메이션이 끝난 뒤에도 항등 행렬로 남아서, 예배 노트는 바가 종이 아래(화면 밖 y≈1089)에 서고 묵상 노트는 아예 안 보였다.
    `createPortal(document.body)`로 빼면 기준이 다시 뷰포트다(덤으로 감싸개의 `onMouseDown`과도 안 얽힌다 · §6-32-y). 바 높이는 **그리기 전에**(`useLayoutEffect`) 재야 한 프레임 깜빡이지 않는다.
