...
- **`showNotification`에 `renotify: true`가 있어야 한다**(`public/sw.js` · 2026-09-27 갤럭시 신고 '푸시는 오는데 진동이 없다') — tag가 `card:<id>`·`link:<주소>`라 같은 업무·같은 주소의 알림이 알림창에 남아 있으면 새 알림이 **소리·진동 없이** 갈아 끼워졌다. `tests/push`가 본다.
...
9-aa. **sticky의 통은 `overflow != visible`로 찾아야 한다** — `auto|scroll`만 보면 `overflow:hidden` 조상을 지나쳐 재는 상자와 붙는 상자가 달라진다(`MarkdownEditor.useStickyTop` ·
    편집기 자신·`window.resize`도 듣는다). **바는 폭과 상관없이 한 벌이다**(사용자 결정 2026-09-11 — "그냥 데스크톱처럼 똑같은 방식으로 모바일·태블릿에서도 되게"): 붙으면 **상자가 아니라 줄**이
    된다 — 둥근 모서리·좌우·위 선을 걷고 아래 선 하나와 `shadow-soft`만 남기며 배경은 `useStickyTop`이 재 온 **통의 바탕색**이다(상수로 박으면 업무 창과 페이지 노트 중 한쪽이 틀어진다 ·
    150ms `--ease-out-quint` · **불투명이고 블러는 없다**). 붙는 자리는 통 안 머리줄의 아래끝이고, 머리줄이 통 **밖**이면 `-paddingTop`이라 통의 테두리 상자 위다 — 모바일 페이지는 그 자리가
    곧 `MobileTopBar` 바로 밑이고(`MobileTopBar`는 `main`의 형제다), 모바일 풀스크린 업무 창은 그 창 머리줄(`shrink-0`) 바로 밑이다. 폭 여섯 × 자리 셋 매트릭스가 `tests/handoff`에 있다.
    **그 전에 모바일만 화면 아래(키보드 위) 고정으로 갈랐다가 걷었다** — 9-aa-4.
9-aa-2. (빈 번호 — 걷어낸 모바일 고정 갈래에 딸린 함정이었다. 번호는 코드가 가리키므로 그대로 둔다 · 9-aa-4)
9-aa-3. **'붙었나'는 관찰자(IntersectionObserver)가 아니라 바와 센티넬의 거리로 잰다**(2026-09-12). 관찰자는 `root`·`rootMargin`·통의 padding을 전부
    맞춰야 했고(2026-09-11에 `root`를 통으로, 아래 margin을 `100000px`로 열어 고쳤는데도) 페이지 통에서는 판정이 뒤집힌 채 굳는 일이 있었다(모바일 390에서
    붙었는데 상자, 안 붙었는데 줄 — Puppeteer 확인). 거리는 단순하다: 센티넬은 바 바로 위 1px 줄이라 안 붙으면 둘의 `top` 차이가 1px, 붙으면 바만 멈춰 벌어진다.
    스크롤은 **`document`에서 capture로** 듣는다(scroll은 버블링하지 않지만 capture는 어느 통이든 지나간다 — 통을 찾아 거기 걸면 통이 늦게 잡혀 창에 걸린다).
    **바는 잴 때마다 찾는다** — `useEditor`가 첫 렌더에서 null이라 Toolbar가 아직 없고, 그때 ref를 한 번만 잡으면 영영 null이다(`data-stuck`이 0에 굳어 있던 원인).
    `data-stuck="0|1"`이 그 상태를 그대로 내놓으니 검사·디버그는 그것을 보면 된다.
9-aa-4. **서식 바를 모바일에서 키보드 위에 고정하려고 했으나 걷었다**(사용자 결정 2026-09-11). 하루 안에 `bottom` → `top` → `body` 포털까지 갔는데 실기기 아이폰에서는 바가 끝내 키보드 위에
    서지 않았다(스크린샷 — iOS 키보드 보조 줄 뒤/저장 버튼 아래의 애매한 자리). `visualViewport` 계산이 헤드리스의 가짜 값과 실기기에서 다르게 움직였다는 뜻이다. 그래서 **모든 폭에서 sticky-top
    하나**로 합쳤다(9-aa) — 폭 분기·포털·`visualViewport` 리스너·포커스 때만 보이기·편집 칸 `padding-bottom`이 전부 빠졌다. `tests/handoff` 매트릭스가 **body 직계 자식 포털이 없다**를
    단정해 되돌아가는 것을 막는다. **다시 시도한다면 실기기에서 `visualViewport.height`·`offsetTop`이 키보드가 열리고 닫힐 때 실제로 어떻게 변하는지 로그로 먼저 확인할 것** — 그 값을 모르는 채로
    계산식을 고치는 일은 이미 세 번 헛돌았다. 곁다리로 남은 사실 둘: ① iOS 사파리의 `position: fixed`는 레이아웃 뷰포트 기준이고 키보드가 열려도 `innerHeight`가 그대로일 수도 줄 수도 있다.
    ② **transform이 걸린 조상이 있으면 fixed의 기준이 뷰포트가 아니다**(§6-1과 같은 함정) — 화면 뿌리 `.dc-screen`의 등장 애니메이션이 끝난 뒤에도 항등 행렬로 남는다.
9-aa-5. **고친 방법(2026-09-18)** — 바를 건드린 게 아니라 **화면 뿌리 높이를 `visualViewport.height`로 따라가게** 했다(`App.jsx`가 그 값을 `--app-vh`로 내보내고 뿌리가 `h-[var(--app-vh,100dvh)]`).
    아이폰(standalone PWA)은 키보드가 올라와도 `100dvh`를 안 줄이고 **보이는 창만 줄인 뒤 화면을 위로 미는데**, 뿌리가 보이는 창만큼이면 밀어낼 것이 없어 sticky-top 바가 제자리에 선다.
    **상수를 계산하지 않고**(9-aa-4가 적어 둔 헛돌이가 그것이다) **transform도 쓰지 않는다**(위 ② — fixed 자손이 어긋난다). `interactive-widget=resizes-content`는 사파리가 안 듣는다
    (2026-09-18에 넣었다 되돌렸다 — 크롬 안드로이드에만 듣는다). 검사는 `tests/mobbits`가 그 변수를 흔들어 본다. **사용자 실기기 확인 완료**("서식바는 일단 완료").
9-aa-5-a. **줄어든 높이가 남으면 어느 화면이든 아래 절반이 빈다**(2026-09-27 신고). `--app-vh`는 키보드가 내려갈 때 오는 resize로만 되돌아오는데
    그 이벤트가 안 오는 때가 있다(홈 화면 웹앱 · 앱을 오갈 때) — 그러면 키보드 높이만큼(대략 절반) 줄어든 뿌리가 남아 스크롤도 그 안에서만 된다.
    **입력칸에 포커스가 없으면 `innerHeight` 아래로 줄이지 않는다**(`App.jsx` `visibleH`) + `focusout`·`visibilitychange`·`pageshow`에서 다시 잰다. 배율도 곱한다(손가락 2배 확대면 보이는 창이 정확히 절반). 검사 `tests/mobbits`.
9-aa-6. **저장 형식은 빈 줄을 합치지 않는다**(2026-09-25 · `markdown.js`) — 빈 문단 N개 = 줄바꿈 N+1개. 끝은 **줄바꿈만** 걷는다(`\s+$`로 걷으면 끝의 빈 항목 `- `가 `-` 글자가 됐다).
    옛 저장은 빈 줄을 하나로 접고 도막 첫 줄 들여쓰기를 지워서, 저장·다시 열기마다 줄이 움직였다(`tests/mdcheck`).
9-aa-7. **블록 안 줄바꿈은 공백으로 적는다**(제목·목록·체크) — 마크다운 한 줄이 한 블록이라 줄바꿈을 그대로 적으면 다시 읽을 때 블록이 갈린다. 편집기도 그 자리의 Shift+Enter·Mod+Enter를 Enter처럼 한다(`BlockBreaks`)
    · 문단 안 Shift+Enter는 줄바꿈 그대로. **중첩 목록은 평평하게 적고 목록 안 Tab은 들이지 않는다**(읽는 쪽이 들여쓰기를 안 본다) · 이어진 번호 줄은 읽을 때의 차례로 적는다(`renumberRuns`).
9-aa-8. **문법처럼 생긴 문단 줄은 `\`로 막는다**(`needsEscape`·`unescapeLine`) — 평문으로 친 `# `·`- `·`1. ` 줄이 저장 뒤 제목·목록이 됐다. 읽는 셋(`mdToDoc`·`RichText`·paper `paperBlocks`)이 같은 함수를 쓴다 —
    **새 읽는 자리를 만들면 `unescapeLine`을 먼저 부를 것**(안 부르면 `\`가 글자로 찍힌다). 이미지 주소 단독 줄은 막지 않는다. 평문 붙여넣기는 `clipboardTextParser`(줄마다 문단 · 빈 줄은 빈 문단).
9-aa-9. **노트 도막 사이 줄은 그대로 되적는다** — `ensureNoteSections`는 `splitRaw`로 도막 사이를 손대지 않고, 종이용 `splitNoteSections`는 도막 앞뒤 **빈 줄만** 걷는다(첫 줄 들여쓰기는 남는다).
    잠긴 도막 바로 뒤 빈 문단(다음이 제목·끝)에서 Backspace·Delete는 아무 일도 안 하고, 지워지면 appendTransaction이 빈 문단을 도로 끼운다(`LockedHeadings`) — 그 줄이 없으면 도막 아래에 커서를 둘 데가 없다.
9-aa-10. **개발 모드 StrictMode는 effect를 두 번 돌린다** — 예배 노트 `MyNote`가 두 번째에 되살린 초안을 서버 note로 덮었다. 같은 주보에서 note만 새로 오면(공유 토글·캐시 뒤 조회) 고치던 글·편집 모드를
    건드리지 않고, **같은 note 객체면 건너뛴다**. 초안은 1.2초 안에 떠나도 열쇠를 떠날 때 `pendingDraft`를 쓴다(말씀·예배 둘 다).
9-aa-11. **읽기 종이 = 편집 화면**(사용자 결정 A1 · 2026-09-25) — 값은 `.note-paper .tiptap`에서 온다: 블록 사이 4px · li 2px · 체크 항목 아래 4px(편집 쪽 `li > div > p`의 margin) · 빈 줄 = 한 줄(`paper-gap`) ·
    break-spaces · 두 열에서 첫 블록 min-height 29px(편집 격자 행이 라벨 높이로 서는 것) · 한 열에서 라벨 line-height 1.6 · row-gap 3px. **`.paper-note` 안쪽만**(주보 종이 불변). **편집 쪽 CSS를 바꾸면 이 값도 같이**
    (`tests/word`·`tests/handoff`가 두 폭에서 줄 위치 차 0을 잰다). 업무 보기(`RichText`)도 빈 줄 = 빈 문단 하나, 문단은 break-spaces.
...
9-as. **손대지 않은 템플릿은 빈 노트다**(`noteTemplate.isTemplateOnly`) — 아니면 아무도 안 쓴 제목 줄이 저장되어 나눔·잔디에 오른다. **옛 도막 이름은 `LEGACY_SECTIONS`에 쌓는다**('본문'까지 — 갈아치우면 옛 빈 노트가 '사람이
...
9-bw. **같은 속성의 Tailwind 유틸리티 둘이 한 요소에 붙으면 이기는 것은 CSS에 나온 차례다**(v4: `rounded` < `rounded-[Npx]` < `rounded-full` < `-lg` < `-md` < `-none` < `-sm` < `-xl` < `-xs`) — 클래스 문자열의 순서가 아니다.
    그래서 **임의 값을 테마 이름으로 바꾸면 겹친 자리의 승자가 바뀐다.** `Skeleton`은 자기 `rounded-md`를 붙여서, 부르는 쪽이 넘긴 `rounded-[4px]`·`[5px]` 여섯 곳이 지금 8px로 그려진다(바꾸면 모양이 바뀐다 · HANDOFF §2 ★).

9-bx. **hover로만 드러나는 조작을 `md:opacity-0`으로 숨기면 아이패드에서 사라진다**(md 이상인데 hover가 없다) — `pointer-fine:md:opacity-0`(Tailwind 4.3에 들어 있다). 지금 쓰는 곳: 참고 링크의 자물쇠·X(`links.jsx`) ·
...
17-d. **포털로 옮긴 목록은 React 트리로는 여전히 자식이다** — 합성 이벤트가 앵커 쪽 조상으로 버블된다(업무 창 딤의 onMouseDown이 목록 안 누름도 받는다 · 대상이 딤이 아니라 닫히지는 않는다).
    DOM `contains`로 판정하는 바깥 누름 훅은 반대로 목록을 '바깥'으로 본다 — `useDismiss`에 목록 ref를 같이 넘기는 이유(§6-0과 같은 뿌리).
...
19-b. **낙관적 갱신과 서버 목록을 '이어 붙이면' 그 사이 프레임이 거짓말한다** — 나눔 피드에 내 글이 두 줄로 섰다. `wordView.mergeFeed(shared, mine)`이 내 줄을 언제나 내 상태에서 한 줄만 만들고 키는 `'mine'`
    고정이다.
19-b-1. **나눔은 종이 하나 + 사람 칩이다**(사용자 결정 2026-09-13 — "그 종이 전체를 보여줘야지 … 쌓이지 않는 구조가 중요"). 사람마다 줄로 쌓고 도막을 라벨|글 두 칸으로 접던 요약(`NoteDigest`)은 지웠다. 이유 둘:
    **공유되는 것은 요약이 아니라 그 사람이 쓴 종이**이고(이미지로 나가는 것과 같은 모양이어야 한다), **쌓이면 사람 수만큼 화면이 길어진다**. 종이는 `paper.jsx`의 `NoteSheet`를 '내 묵상' 읽기 종이와 **같은 부품·같은 폭**(`QT_SHEET_BOX`)으로
    세운다 — 여기에만 다른 마크업을 두면 한쪽만 고쳐진다(§6-32-p). 고른 사람은 **순번이 아니라 id로** 든다(남이 하나 올리면 보고 있던 종이가 남의 것으로 바뀐다). '나만 보기' 잠금 표시는 줄이 아니라 **내 칩**에 붙고, 칩 줄은 넘치면 가로로
    민다(`x-scroll-lock` · §8의 '같은 종류가 이어지는 줄'). 도막 없이 쓴 옛 나눔은 `splitNoteSections`가 라벨 없는 도막 하나로 주므로 종이가 그대로 선다.
19-c. **map 안에서 바깥 변수와 같은 이름을 쓰면 잠금이 조용히 풀린다** — 출석의 `open`(펼침)이 13:30 게이트를 이겨 칩이 하나도 안 잠겼다 → `checkOpen`으로 갈랐다.
19-d. **편집 목록의 `key={i}`도, 입력마다 바뀌는 열쇠도 안 된다** — 줄을 지우면 아래 칸이 위 줄의 글을 물려받고, 입력 중에 열쇠가 바뀌면 칸이 새로 마운트되어 글자마다 포커스를 잃는다.
    `utils.stableRowKeys`: 같은 객체 → 그 열쇠 · 그 자리에서 고친 줄 → 자리 열쇠 · 새 줄 → 새 열쇠(주보 편집의 담당자·찬양·광고 · `tests/logcheck`).
...
24-c. **딥링크의 나머지 값은 App이 주소를 `/`로 정리하기 전에 붙잡아야 한다** — `services/entryQuery.js`가 모듈 첫 실행 때 스냅샷을 뜬다. **OAuth 왕복은 그 스냅샷보다 늦다** → `auth.consumeReturnTo`가
    다시 실어 준다.
24-c-1. **딥링크 값 이름을 화면끼리 겹치게 짓지 마세요** — 종을 누르면 `setEntryQuery`가 값을 넣고 **지금 떠 있는 화면의 진입 이펙트가 먼저 돈다**.
    예배 화면이 떠 있을 때 `/?p=groups&s=<주보>`를 누르면 예배 화면이 `s`를 집어 그 주보 상세를 열고 나서야 모임으로 간다. 그래서 모임 화면의
    주보 값은 `note`(공유 노트)·`guide`(순모임 가이드)다(2026-09-25).
...
29-f. **드라이브 왕복은 느리다 — 시간 제한을 우리가 고르세요**(바닥값 4초, Apps Script 자체 비용). `vercel.json`으로 `api/drive.js` = 60초를 **명시**하고 `api/drive.js`가 **55초에 스스로 끊는다**(`SCRIPT_BUDGET_MS`)(안
...
29-z-9. **한글 PDF는 `cMapUrl` 없이는 글자가 통째로 빈다**(`PdfView.jsx`) — 한글 PDF는 글꼴을 CID로 품고, pdf.js는 그것을 풀 때 `cmaps/Adobe-Korea1-UCS2.bcmap`을 받아야 한다. 못 받으면 경고 한 줄만 내고
    **그 글꼴을 버린다** — 숫자·영문은 다른 글꼴이라 남아서 "반쯤 그려진 종이"가 된다(사용자 신고 2026-09-13). 자료는 `node_modules/pdfjs-dist/`에 이미 있고 `vite.config.js`의 `pdfjsAssets`가
    `/pdfjs/{cmaps,standard_fonts,wasm,iccs}/`로 내준다(dev는 미들웨어, build는 결과물 — public/에 복사해 커밋하면 169개가 눌러앉고 판을 올릴 때 어긋난다). **두 곳이 같은 칸을 봐야 한다** — `tests/drivesync`가 본다.
29-z-10. **blob: 주소는 홈 화면 앱·인앱 웹뷰의 iOS 사파리에서 내려받기가 안 된다** — `a.download` + `URL.createObjectURL`이 저장 대신 "'미리보기'에서 열기" 페이지를 띄웠다(사용자 스크린샷 2026-09-13).
    내려받기는 **`Content-Disposition: attachment`가 달려 오는 진짜 주소**로 보낸다(`cloud.getFileDownloadUrl` — 드라이브는 `uc?export=download`, Storage는 서명 URL + `download`). 그러면 2026-08-28의
    "남의 출처에서 `download` 속성이 무시된다"도 같이 없어진다(그 속성이 아니라 머리줄이 일을 한다). blob은 **아직 올리는 중이라 주소가 없는 파일** 하나에만 남는다.
29-z-11. **`x-scroll-lock`(touch-action: pan-x)으로 iframe을 감싸면 틀 안에서 세로로 못 읽는다** — 칩 줄을 위해 만든 유틸리티인데 구글 문서 미리보기 겉에 둘렀더니 손가락으로 문서를 내려 읽을 수가 없었다
    (감사 2026-09-13). 가로 넘침만 막을 때는 `overflow-x-auto overscroll-x-contain`이다. 같은 자리의 940px 넓히기는 **구글 문서(A4 종이)에만** — 슬라이드 `embed`는 제 틀에 맞춰 줄여 그려서 넓히면 오히려 잘린다.
29-z-12. **틀로 그리는 갈래는 전부 시간을 재야 한다** — `onLoad`가 영영 안 오는 환경이 있고(서드파티 프레임 차단·콘텐츠 차단기) 그때 스켈레톤이 그대로 남는다. 'office' 하나만 재고 있었는데 첨부가 드라이브로
    옮겨진 뒤 그 갈래는 도달하지 않았고, 'drive' 가지는 `timedOut`을 **읽기만** 하고 타이머가 안 켜져 있어 지키는 척만 했다(감사 2026-09-13). 시간이 지나도 **틀은 걷지 않는다**(늦게 뜨면 살아나야 한다) —
    덮개만 '새 탭에서 열기'로 바꾼다. 막다른 길(누를 것 없는 안내 두 줄)을 남기지 마세요.

29-z-13. **`index.html`의 `maximum-scale=1.0`이 안드로이드에서 손가락 확대를 통째로 막는다**(iOS는 접근성 때문에 무시한다) — 그래서 폰에서는 사진 하나 키울 길이 없었고, '화면 가득'은 데스크톱 전용이다.
    미리보기 창의 확대(이때는 단계식 `ZOOM_STEPS` — **지금은 연속 배율이다 · z-13-a**)는 **우리가 그리는 갈래(사진·PDF)에만** 붙는다 — 구글 틀·오피스 뷰어는 안쪽에 손댈 수 없고 저희 확대가 이미 있다. PDF는 CSS로 늘리지 않고
    **그 배율로 다시 그린다**(글자가 뭉개지면 확대하는 뜻이 없다) — 대신 캔버스 실제 픽셀에 상한을 둔다(`MAX_CANVAS_PX_W`). 사진을 확대할 때는 **flex를 걷어야 한다**: `justify-center` 통에서 내용이 넘치면
    시작 쪽이 잘려 거기로 스크롤이 안 된다.

29-z-13-a. **계단식 배율을 연속 배율로 바꾸면 손댈 자리가 넷이다**(사용자 결정 2026-09-14 — "150%, 200% 붙이지 말고 손가락으로 펴고, 마우스로 휠로"). `ZOOM_STEPS`·`nearest()` 스냅과 머리줄 `－ 100% ＋` 버튼을 걷었다.
    ① **안드로이드에는 `gesturestart`가 없다**(그건 사파리 전용이다). 거기서는 `touchstart`/`touchmove`로 **두 손가락 사이 거리를 직접 재야** 한다 — 안 하면 `maximum-scale=1.0`(z-13)이 브라우저 확대를 막고 우리 배율도 안 움직여서 **확대할 길이 아예 없다**.
    ② **사파리는 두 길을 다 준다.** 순서가 `touchstart`(2) → `gesturestart` → `gesturechange`/`touchmove`라, `gesturestart`에서 깃발을 세우고 `touchmove` 맨 앞에서 그 깃발을 보면 배율이 두 번 걸리지 않는다. 기능 감지가 아니라 **이벤트가 실제로 왔을 때만** 꺼야 한다(있다고 광고하고 안 쏘는 브라우저가 있다).
    ③ **데스크톱은 `ctrlKey || metaKey`인 `wheel`만** 배율이다 — 그냥 휠까지 먹으면 긴 PDF를 읽을 수 없다. 트랙패드 오므리기는 `ctrlKey: true`인 wheel이라 같은 길로 따라온다.
    ④ **PDF는 손이 멎은 뒤에 다시 그린다**(`ZOOM_SETTLE`) — 연속 배율에서 매 프레임 50쪽을 래스터화하면 폰이 멈춘다. 그 사이에는 이미 그린 캔버스를 CSS로만 늘린다(잠깐 부드러워졌다가 또렷해진다). 그리고 **비우기 전에 스크롤 자리를 비율로 기억**해야 한다 — 안 그러면 확대할 때마다 1쪽으로 튄다.
    그리고 `zoom > 1` 판정이 손 떨림에 깜빡이면 스크롤 모드·끌기 배선이 붙었다 떨어졌다 하므로, **하한 근처는 정확히 1로 붙인다**.

    ⑤ **연속 배율은 손짓이 도는 동안 상태를 건드리면 안 된다**(사용자 신고 2026-09-17 — "손가락으로 줌인 줌아웃 하면 프레임 단위로 끊겨가지고"). `touchmove`마다 `setZoom`을 부르면 **매 프레임 창 전체가 다시 그려지고**, 사진은 감싸개 폭이 `zoom × 100%`라 레이아웃까지 다시 잡는다(PDF는 캔버스 쉰 장). 폰에서 그 한 바퀴가 16ms를 넘기면 손가락을 못 따라와 계단처럼 보인다.
    고치는 길은 **손짓 동안에는 내용 층(`data-zoom-layer`)에 `transform: scale()`만 먹이고 스크롤을 직접 쓰고, 손가락이 둘 미만이 되는 순간 한 번만 커밋**하는 것이다(합성만 돌아 레이아웃 0). 놓치기 쉬운 것 셋:
    · transform은 **새 배율이 그려진 뒤**에 걷는다(`useLayoutEffect` 맨 앞) — 먼저 걷으면 한 프레임 동안 옛 배율이 번쩍인다.
    · 커밋은 **두 번 불려도 한 번만** 먹어야 한다 — 사파리는 `gestureend`와 `touchend`를 둘 다 준다.
    · 걸리는 자리는 **통이 아니라 내용 층**이다(통에 걸면 스크롤까지 같이 늘어난다). PDF는 쪽 틈도 배율을 따라가야 한다(`PAGE_GAP × 배율`) — 고정이면 transform이 늘린 틈과 어긋나 손을 뗄 때 화면이 튄다. 쪽 테두리 1px은 아직 따라가지 않아 여러 쪽을 크게 볼 때 수십 px이 어긋날 수 있다(3배에서 테두리가 3px이 되는 쪽이 더 나빠 보류했다).

    ⑥ **손짓 동안에는 통의 스크롤을 쓰지 마세요 — `translate`로 주세요**(사용자 신고 2026-09-17 둘째 — "확대가 밑으로 내려가는데... 어디 걸린 것처럼"). ⑤로 `transform`은 붙였는데 기준점 보정을 `scrollLeft/Top`으로 했더니, **맞춤(배율 1)의 통은 `overflow-hidden`이라 그 값이 0으로 잘렸다.** 보정이 통째로 죽어 사진이 그냥 아래로 미끄러진다. 데스크톱 크롬에서는 같은 코드가 멀쩡히 먹어서 **폰에서만 났다.**
    옮길 만큼은 `translate(dx, dy) scale(k)`의 translate로 준다(`dx = 지금 스크롤 − 목표 스크롤`, 목표는 커밋된 배율에서의 값이고 **끝을 넘지 않게 자르는 것까지 같이**). 손을 떼면 그 목표값이 그대로 커밋 기준점이 되어 진짜 스크롤이 된다. 통의 `overflow`는 안 건드린다 — 맞춤에 스크롤바가 생기면 안 된다.
    **가운데 정렬은 범인이 아니다**(짐작했다가 실측으로 반증했다): 층이 `zoom × 100%`이고 사진이 그 안에서 가운데여도 `max-w/max-h`가 무는 한 사진 상자는 자리·크기가 배율에 정확히 비례해서, 원점 `0 0` scale이 커밋된 레이아웃을 그대로 재현한다(여덟 경우에서 크기 차 0px·자리 차 0.7px 이내).
    그리고 **사파리 `GestureEvent.clientX/Y`가 비어 올 수 있다**고 보고 빈 값이면 칸 한가운데·직전 터치 자리로 떨어지게 해 두었다 — 비면 셈이 NaN이 되어 같은 증상이 난다.

    ⑦ **다시 그릴 때 먼저 비우면 하얗게 끊긴다**(사용자 신고 2026-09-17 — "중간중간 프레임이 끊긴 것처럼 하얗게 한 0.01초"). `replaceChildren()`로 비우는 순간 종이가 없어 바탕이 드러나고, 재어 보면 `scrollHeight`가 통째로 접힌다(2688 → 640, `scrollTop` 800 → 0).
    같은 문서·같은 칸 너비면 **비우지 말고 쪽마다 다 그린 뒤 제자리에서 갈아 끼운다**(`replaceChild`). 캔버스는 **붙이기 전에** 그린다 — 붙여 놓고 그리면 쪽마다 빈 종이가 한 번씩 보인다. 두 벌이 같이 사는 순간이 없어 메모리도 쪽 수 + 1장이다.
    갈아 끼우는 중에 배율이 또 바뀌면 **두 배율의 쪽이 층에 섞이므로**, 쪽마다 그려진 배율을 적어 두고(`dataset.z`) CSS 확대가 **쪽마다 제 배율에서** 곱해야 한다. 칸 너비가 바뀐 때(화면 가득·회전)는 쪽 크기 자체가 달라지므로 예전처럼 비우고 비율 스크롤을 복원한다 — **제자리 교체 중에는 스크롤을 건드리지 않는다**(읽던 자리를 뺏는다).
    `mx-auto`도 같이 걷었다: 쪽이 칸보다 넓어지는 순간 0으로 접혀서 여백까지 늘리는 transform과 어긋나고, 손을 뗄 때 종이가 옆으로 뛴다. 좌우 여백도 `PAGE_SIDE × 배율`이다.

    ⑧ **확대 층과 그 안의 그림이 어긋나면, 손을 떼는 순간 도로 줄어든다**(사용자 신고 2026-09-17 셋째 — "확대되면 되는거지 다시 축소되는 현상은 왜 나오는거지"). 원인 둘을 **진짜 컴포넌트에 손가락 동작을 먹여** 잡았다(합성 상자로만 재다가 놓쳤다).
    · `<img>`에 `max-w-full max-h-full`만 주면 **제 원래 크기까지만** 커진다. 층이 그보다 커진 순간부터 그림이 안 따라오는데, 손짓 동안에는 `transform`이 층째로 늘려 보여 주므로 **뗄 때 도로 줄어든다**(작은 그림은 2.5배 → 1배로 튀었다). 큰 그림도 층과 몇십 픽셀 어긋나 그만큼 빈자리가 스크롤 범위에 남았다(가로로 긴 그림 75px). 층이 이미 그림 비율이니 `w-full h-full object-contain`이면 딱 맞는다.
    · 칸 크기를 `getBoundingClientRect()`로 재면 **창이 뜰 때의 등장 애니메이션(`zoom-in-95`)에 95%로 잡힌다.** 맞춤 크기가 그만큼 작게 정해지고, 확대 뒤 스크롤바가 생겨 옵저버가 다시 도는 순간 제 크기로 고쳐지며 **그림이 5.3% 튄다.** `offsetWidth/Height`는 레이아웃 크기라 transform을 안 타고 스크롤바가 생겨도 그대로다.
    그리고 밀 수 있는 끝은 **층에서** 센다(`Math.max(offsetWidth, scrollWidth)`) — PDF 층은 블록이라 제 폭이 칸 폭이고 쪽이 그보다 넓게 삐져나가므로 `scrollWidth`가 진짜 폭이다.
    재는 방법: 임시 페이지에 진짜 `FilePreviewModal`을 띄우고 CDP `Input.dispatchTouchEvent`로 두 손가락을 벌린다. **손을 떼기 직전과 뗀 뒤의 그림 크기 비가 1.000이어야** 한다. 세로로 긴·가로로 긴·작은·정사각 넷 × 데스크톱·모바일 × (확대·상한 넘기기·다시 오므리기)를 다 봐야 한다 — 셋 중 하나만 봐서 두 번 놓쳤다.

29-z-13-b. **종이(공유 그림)에 바깥 출처 사진이 들어가면 굽는 두 길을 다 봐야 한다**(2026-09-14). 기본은 `modern-screenshot`(foreignObject)이고 대비용이 `html2canvas`인데(§6-32-t), foreignObject는 **사진을 data URI로 심지 않으면 그 자리가 빈다**.
    크롬에서는 두 길 다 스스로 인라인하지만(`embedImageElement` · `useCORS`), 실패해도 **경고만 찍고 빈 자리로 굽는다** — 그림에 사진이 없는 채로 나가도 아무도 모른다. 그래서 `shareImage.inlineImages`가 굽기 전에 바꿔 심고 끝나면 되돌린다.
    Supabase Storage 공개 주소에는 `access-control-allow-origin: *`가 붙는다(확인함). **사파리는 재어 보지 못했다** — 실기기 확인 항목이다.

29-z-14. **두 번 눌러 확대하는 자리는 브라우저의 더블탭 확대를 먼저 끊어야 한다**(`tap-zoom-lock` · index.css) — 안 끊으면 우리 배율만 바뀌는 게 아니라 **브라우저가 페이지 자체를 확대**한다. 그러면 `fixed`로 깔린 창이
    보이는 영역보다 커져서, 배율을 되돌려도 버튼이 화면 밖에 남고 "눌리지 않는" 것으로 보인다(사용자 신고 2026-09-13 — "확대했다가 축소하면 다른 버튼이 안 눌리고 업무 상세 밖으로 나가지지도 않는다").
    `touch-action: manipulation`은 밀기·손가락 확대는 그대로 두고 더블탭만 끊는다(`.board-card`가 쓰던 그 값이다). 길게 눌러 뜨는 콜아웃도 같이 막는다 — 두 번째 탭이 그 UI를 닫는 데 쓰여서 한 번에 안 눌린다.
    **iOS는 `maximum-scale=1.0`도 `touch-action`도 무시하고 손가락 확대를 허용한다**(접근성). 그것까지 막는 길은 사파리가 주는 `gesturestart`를 `preventDefault`하는 것 하나뿐이라, 미리보기 창은 그 이벤트를 받아
    **우리 배율로 돌린다**(`gesturechange.scale` → 연속 배율 · z-13-a). 안 막으면 "확대한 뒤 밀면 버튼이 아예 안 먹는다"가 된다(사용자 신고 2026-09-13 두 번째) — `fixed` 창은 레이아웃 뷰포트에 붙어 있는데
    사람이 보는 것은 좁아진 비주얼 뷰포트라, 밀면 머리줄이 보는 화면 밖으로 나간다. **우리 배율이 있는 갈래(사진·PDF)에서만 막는다** — 구글 틀에서 막으면 대신 줄 것이 없어 키울 길을 통째로 뺏는 셈이다.
    확대한 통에는 `overscroll-contain`도 같이 — 끝까지 민 뒤 계속 밀면 스크롤이 뒤 화면으로 넘어가 다음 탭이 엉뚱한 데 떨어진다.
29-z-15. **pdf.js 6은 아주 새 표준 메서드를 확인 없이 그냥 쓴다 — 없으면 PDF가 한 장도 안 그려진다**(`services/pdfPolyfill.js`). 크롬 131에서 `toHex` → `getOrInsertComputed` → `bytes`가 차례로 터졌다.
    가장 늦은 것이 사파리 26이라 **고치기 전에는 iOS 18에서 첨부 PDF가 통째로 막혀 있었다**(화면에는 "미리보기를 그릴 수 없어요"만 떠서 파일 탓으로 보인다). **폴리필은 앱과 워커 두 곳에 각각** 들어가야 한다 —
    `pdf.worker`는 다른 전역이다. 그래서 `workerSrc`가 pdf.worker를 바로 가리키지 않고 껍데기(`pdfWorkerEntry.js` · `?worker&url`)를 지나며, 거기서 **폴리필을 워커보다 먼저** import한다(순서가 뒤집히면 이미 터진 뒤다).
    `Response.bytes()`는 cmaps·standard_fonts를 받는 길이라 이것이 없으면 §6-29-z-9의 한글 빠짐이 옛 브라우저에서 그대로 재현된다. pdfjs-dist를 올린 뒤 PDF가 안 열리면 **빠진 메서드가 또 있는지부터** 보세요.
29-z-15-a. **pdf.js 6.3은 모듈 맨 위에서 `Iterator`를 본다** — 전역 `Iterator`가 없는 브라우저(사파리 18.4 미만·크롬 122 미만)에서는 메서드가 아니라 **모듈 로드에서** ReferenceError로 PDF가 통째로 죽는다.
    `pdfPolyfill.js`가 `Iterator` 전역과 도우미·`Promise.try`·`Math.sumPrecise`를 채운다. **pdf.js를 부르는 자리는 전부 그 폴리필과 `pdfWorkerEntry` 워커를 거친다** — 첨부 발췌(`fileText.js`)가 폴리필 없는 워커로 `GlobalWorkerOptions.workerSrc`를 덮어 미리보기까지 그 워커를 썼다(`tests/drivesync`).

29-z-16. **그림을 마우스로 끌면 브라우저가 '이미지 끌어놓기'를 시작하고, 그동안 `mouseup`·`click`이 통째로 사라진다** — 놓은 자리에 따라 창이 닫히거나 **아무 버튼도 안 먹는 것처럼 보인다**(사용자 신고 2026-09-13 셋째 —
    "마우스로 사진을 드래그하려고만 해도 다른 버튼이나 화면 동작을 안 해"). 데스크톱·모바일 모두, **확대와 상관없이** 늘 그랬다. 고침은 두 짝이다: 그림에 `draggable={false}`, 그리고 끌기를 **밀기로** 받는다
    (`media.usePanDrag` — 손가락은 브라우저가 이미 밀므로 `pointerType === 'mouse'`만). **`pointerdown`에서 `preventDefault`를 부르지 마세요** — 뒤따르는 `click`·`dblclick`이 같이 사라져 '두 번 눌러 확대'가 죽는다.
    같은 자리의 짝: **창 딤은 '누른 곳도 딤일 때만' 닫는다**(`downOnDim`) — 안 그러면 사진을 끌다가 딤에서 손을 떼는 순간 `click`이 **두 곳의 공통 조상**에서 나서 창이 제멋대로 닫힌다(업무 창이 이미 쓰던 판정이다).

29-z-17. **`vercel.json`에 `regions`를 안 적으면 함수가 기본값(`iad1` 워싱턴)에서 돈다 — DB는 서울이다.** 그러면 요청이 태평양을 **두 번** 건넌다(한국 → 워싱턴 함수 → 서울 DB → 워싱턴 → 한국).
    실측 2026-09-14: 정적 파일 70ms · 함수만 270ms · **함수+DB 860ms**(첫 호출 2.1초는 콜드스타트). 그 860ms 중 600ms쯤이 순수한 거리값이었다. `"regions": ["icn1"]` 한 줄로 서울 안에서 끝난다.
    확인은 응답 머리줄 `X-Vercel-Id`다 — `icn1::iad1::`은 "서울로 들어와 워싱턴에서 실행", `icn1::icn1::`이 맞는 모양이다. **Supabase를 옮기면 이 값도 같이 옮긴다**(`tests/drivesync`가 못 박는다).
    `iad1`이 제일 붐비는 리전이라 함수가 덜 식는 이점은 잃는다 — 다만 iad1에서도 첫 호출이 2.1초였다.

29-z-18. **첫 화면 벤더를 따로 칸에 낼 때 `node_modules`를 통째로 묶으면 첫 화면이 2배가 된다**(`vite.config.js`의 `EAGER_VENDORS`) — lazy로 잘 빠져 있던 tiptap 286 · pdfjs-dist 425 · jspdf 332 ·
    html2canvas 199 kB가 그 칸에 끌려 들어와 1,096 → **2,052 kB**가 됐다(2026-09-14에 해 보고 되돌렸다). 목록은 **첫 화면에 이미 있는 패키지 이름만** 적는다. 새 의존성을 더하기 전에 그것이 정말 첫 화면에서
    쓰이는지 보세요 — 한 번도 안 여는 사람까지 받게 된다. 노리는 것은 **크기가 아니라 배포 사이의 캐시**다(`/assets/*`가 immutable이라, 앱만 고친 배포에서 vendor 해시가 그대로면 재방문자는 129 kB(gzip)를 안 받는다).
...
31-h. **정책은 행을 보고 칸은 안 본다** — `profiles_update`가 자기 행의 **모든 칸**을 열어 미승인 계정이 스스로 승인하고 `merged_into`를 위조했다(0071). 칸을 막을 때는 **예외 대신 옛 값으로 되돌리는** BEFORE 트리거를 쓴다
    (upsert가 행을 통째로 보낸다) · 통과 조건은 `auth.uid() is null or is_admin()`(psql·서비스 키·가입 트리거). 작성자 칸(`created_by`·`uploaded_by`·`author_id`·`actor_id`)도 같은 모양이다.
31-i. **`ON CONFLICT DO UPDATE`도 INSERT 정책의 with check를 먼저 본다** — update 정책만 넓혀서는 upsert가 여전히 42501이다(합친 계정의 내 정보 저장 · 0071이 `profiles_insert`도 `effective_uid()`로 열었다). `INSERT … RETURNING`이 SELECT 정책을 보는 것은 25번.
31-j. **임베딩 표는 원본보다 넓게 읽히면 안 된다** — `bible_vec`·`doc_vec`은 읽기가 `is_approved()` 하나라, 원본의 읽기 규칙이 그보다 좁은 것을 넣으면 그 글이 전원에게 샌다.
    그래서 **주보 첨부**(작성 중 주보의 파일은 `can_edit_service()`만 읽는다)와 **개인 표**(`service_notes`·`qt_entries` — 본인·순·공개로 갈린다)는 `doc_vec`에 없다. 넣으려면 표를 따로 두거나 RLS를 원본과 같게 한다.
...
42-b. **두 갈래가 같은 파일 끝에 구역을 덧붙이면 cherry-pick 충돌 풀이가 공통 꼬리(닫는 괄호 `});`·`}`)를 하나로 합친다** — 한 구역이 안 닫혀 다음 구역을 삼키거나 문법 오류가 난다(2026-09-24 `logcheck`·`drivesync`).
    합친 뒤에는 **그 스위트를 한 번 돌려** 요약 줄까지 나오는지 본다.
42-c. **검사가 글을 치지 않고 통과할 수 있다** — `tests/worship`의 '취소하면 고치던 글을 버린다'(`버릴 글`)는 `.tiptap.focus()` 뒤에 쳐서 커서가 첫 도막 제목(잠김)에 앉아 **글이 애초에 안 들어간다**.
    지금도 통과하지만 아무것도 재지 않는 줄이다(2026-09-25 편집 감사 · **아직 안 고쳤다** — HANDOFF §2). 편집기에 치는 검사는 **문단에 커서를 놓고** 치고, 친 글이 들어갔는지 먼저 확인한다.
...
45-a. **서식 안의 `@이름`** — 굵게·형광펜 안의 멘션은 뒤에 `**`·`==`가 붙어 이름으로 안 읽혔다. `MENTION_TAIL`에 `*=_~`와 백틱을 더했다(**끝에 붙은 것만** 걷는다).
...
46-a. **예시의 날짜는 무엇이든 베껴진다** — 다듬기 예시 4의 '9월 26일까지'·'4월 25일'이 기한 없는 카드에 옮겨 왔다. 예시에는 원문의 기한 하나만 두고, 날짜 없는 줄을 날짜 없이 보여 준다(`tests/aictx`가 예시 4의 날짜를 센다).
    예시의 사람도 지은 이름이다 — 실명을 두면 모델이 그 사람을 다른 카드로 끌어왔다.
46-b. **본문에 한두 번 나오는 '순'은 순 일이 아니다** — 수련회 피드백의 '## 순장 피드백' 한 도막으로 찬양팀장이 순장님이 됐다. 본문 신호는 세 번 이상 · 업무 팀 직함보다 뒤(`aiPeople.isSunText`).
46-c. **`taskScope`에 본문은 `content`(또는 둘째 인자)로 넘긴다** — `text` 속성으로 넘기면 빈 글을 본다(검사가 그렇게 한 번 헛돌았다).
...
**34-h-2.** **업무 첨부도 같은 길이다**(2026-09-11 · 자격은 올린 사람+마스터+관리자). 다만 **권한을 붙인 뒤에 열어야 한다** — 사본에 편집자가 없는 채로 편집 주소를 열면 구글이 읽기 화면을 주고 그것이 위 신고의 정체다.
    누를 때 **편집 주소를 제스처 안에서 곧장 열고** `grantCopyEditors`(Apps Script v11 `grantEditors` · 멱등)는 뒤에서 붙인다. 처음에는 빈 탭을 열고 권한을 기다렸는데 왕복 3~5초가 "about:blank가 떠 있다"로 보였다(2026-09-11). v11이 사본을 만들 때 편집자를 이미 붙이므로 그 기다림은 옛 사본에만 뜻이 있었다 — 옛 사본은 첫 화면이 읽기일 수 있고 새로고침하면 편집이 된다.
...
**34-h-3.** **본문·참고 링크·큐시트가 함께 쓰는 창(`DocEmbedModal`)도 같은 함정이었다**(사용자 신고 2026-09-20 — "ppt 첨부로 들어간 건 해결됐는데 본문에 들어가 있는 건 또 그 문제가 난다"). 첨부는 `FilePreviewModal`이고 이쪽은
    `components/DocEmbed.jsx` + `services/docEmbed.js`라 2026-09-18 수정이 안 닿았다. **모달 한 곳을 고치면 셋이 같이 낫는다**(`RichText.jsx`·`links.jsx`·`worshipDetail.jsx`가 다 이 창을 연다).
    폰에서는 `docEmbedSrc`가 **보기 주소**를 주고(`/preview` · 시트는 `widget=true&rm=minimal` · `#gid=`는 보존) **`authuser`를 붙이지 않는다** — 34-h ①과 같은 쿠키 분할이라 그 인자가 거꾸로 막았다.
    확인한 것: 그 문서는 로그인 없이도 열린다(`/edit?rm=minimal`·`/preview`·`/embed` 전부 200에 진짜 제목) — **주소·공유 설정이 아니라 iframe 안의 로그인 상태**가 문제다.
    **슬라이드는 폰에서 iframe을 아예 만들지 않는다** — 인증만 고치면 덱이 *실제로 그려지기 시작해* §6-29-y-2로 앱 웹뷰가 죽는다(그래서 지금까지 '액세스 권한 필요'만 뜨고 안 죽었던 것이다 · 구글이 인증에서 먼저 막아 덱을 안 그렸다).
    대신 첨부의 `slide-card`와 **같은 모양**으로 첫 장 그림(`docThumbUrl` — lh3 · 로그인 없이 온다) 한 장과 '새 탭에서 열기' 버튼만 세운다(두 파일은 서로 import하지 않는다). 폰에서 고치는 길은 창 머리줄의 **새 탭**(1차 쿠키)이고, 머리줄 뒷절이 그 말을 한다(사용자가 문장을 직접 정했다).
...
**32-l.** **노트 도막 제목은 저장 자리에서 되살린다**(`noteTemplate.ensureNoteSections`) — 편집기 안에서 손을 막는 대신 **저장되는 글**에서 보장하면 붙여넣기·되돌리기와 안 부딪히고 한 자리만 지키면 된다. 잃는 것이 없어야
    ...
    지금 도막은 예배 노트 **말씀 요약·나의 결단·기도**, QT **나의 결단·기도**다 — **구절은 종이 머리(`PaperNoteHead`)에만 선다**('본문' 도막은 2026-09-12에 뺐고 `LEGACY_SECTIONS`에만 남는다).
...
**32-p.** **노트는 종이 안에서 쓴다**(2026-09-10) — 편집 화면이 읽기 종이와 **같은 부품**으로 선다(`paper.jsx`의 `PaperMast`·`PaperNoteHead`·`PaperSheet`·`NotePaper`가 그래서
    ...
    더는 같지 않았다** — 2026-09-25에 사용자 결정 A1로 읽기를 편집에 맞췄다(9-aa-11). **목록은 예외 하나가 더 붙는다** — 그 `padding-left: 12px`이 `.tiptap ul`의 들여쓰기를 덮어써서 글머리가 칸 밖으로 나갔다(§6-32-u).
...
**32-w.** **구절을 푸는 자리는 조회 한 곳이다**(2026-09-11 · `bibleRef.fullRef`). QT 읽기표는 약자로 저장되어 있어서(`qt_schedule.passage_ref` · 0038 시드) 보여줄 때 책 이름 전체로 푼다. **화면에서 책
    목록(`loadBibleIndex`)을 따로 읽지 마세요** — 목록도 비동기라 구절이 한 박자 늦게 바뀌고, 그 사이에 만들어 둔 노트 템플릿이 **손대지 않은 글을 '사람이 고친 글'로** 만든다(저장 버튼이 열리고 빈 노트가 저장된다). `wordView`의
    `useCached` 안에서 묵상·나눔과 **같이** 풀어 `refFull`로 실어 오면 그럴 틈이 없다. 옛 캐시에는 그 칸이 없으므로 **저장된 글자로 떨어지게** 두고, 다시 읽으면 풀린 값이 온다. 종이·본문 카드·템플릿이 **같은 한 값**을 쓴다.

**32-x.** **노트에 칸을 더하면 `shouldAdoptBody`도 그만큼 늘어난다**(2026-09-11 · 묵상 제목 · 0062). 제목과 글을 **판정 하나로 묶지 마세요** — 제목이 안 바뀐 프레임마다 `next === body`가 참이 되어 **글까지
    안 들어가고**, 반대로 제목만 고쳐도 남이 고친 글이 편집기를 덮는다(§6-24-c). 규칙은 같고 보는 칸만 다른 판정을 **칸마다 하나씩** 둔다(`syncedBody`·`syncedTitle`). 저장·취소·삭제·공유 토글이 전부 그 칸을
    같이 태워야 한다 — 공유 토글이 `title`을 안 실으면 토글 한 번에 제목이 지워진다.

**32-y.** **감싸개의 `onMouseDown`이 틀 안의 입력 칸 포커스를 빼앗는다**(2026-09-11 · 묵상 제목 칸). `MarkdownEditor`의 `focusEnd`는 `.tiptap` **밖**을 누르면 `preventDefault()`로 막고
    커서를 문서 끝으로 보낸다(빈 여백을 눌러도 글이 써지게 한 규칙 · 2026-08-30). 틀(`frame`)이 들어오면서 그 감싸개 안에 편집기 말고 **누를 것**이 생겼는데(종이 위 제목 `<input>` · §6-32-p) 그것까지
    막혀서, 실기기에서 제목을 눌러도 커서가 안 잡혔다("제목 미정 글자만 있고 못 고친다"). **스스로 포커스를 받는 것은 그대로 두세요** — `closest('input, textarea, button, a, [contenteditable], select, label')`.
    **검사는 진짜 마우스로 눌러야 한다**(`Input.dispatchMouseEvent`) — 합성 `MouseEvent`는 `isTrusted`가 아니라 브라우저가 포커스를 옮기지 않아 고치기 전에도 '통과'한다.
...
### 교회력 · 주보 스토리 · 광고 달력 · 내 노트 모아 보기 (2026-09-25)

**32-za.** **인라인 `background` 줄임말은 클래스의 `background-image`를 이긴다** — 카드·상세 머리에 `style={{ background: 'var(--app-surface)' }}`가 있으면 `.season-wash`의 절기 물이 통째로 사라진다(줄임말이 이미지를 none으로 되돌리고
    인라인이 더 세다). 물을 얹는 상자는 **`backgroundColor`**로 준다(`worshipView` `CARD_SEASON_STYLE` · 상세 머리). 표지 사진(다음 배치)도 같은 자리다.

**32-zb.** **`service_notes`의 읽기 정책은 '내 것 OR 같은 순에 공유된 것'이다** — '내 예배 노트'를 모으면서 `profile_id`로 거르지 않으면 순원이 공유한 노트가 내 목록에 섞인다. `worship.fetchMyNotes`가
    `.eq('profile_id', myUid())`를 지키고 `tests/logcheck`가 소스로 단정한다. 쓰기 정책은 내 것만이라 저장 쪽은 이 함정이 없다.

**32-zc.** **`.ics`는 브라우저가 주소를 직접 연다 — Authorization 머리가 없다.** 아이폰 Safari는 `text/calendar`를 **탐색으로** 받아야 캘린더 '추가' 화면을 띄운다(blob은 내려받기로 떨어진다). 그렇다고 접근 토큰을 주소에
    실으면 방문 기록·서버 로그에 남는다. `api/ics.js`는 두 걸음이다 — 로그인 POST가 승인을 보고 **10분짜리 HMAC 서명 주소**를 주고, GET은 서명·만료만 본 뒤 **같은 파서**(`noticeDate.js`)로 광고를 다시 읽는다.
    안드로이드는 이 길을 안 탄다(구글 캘린더 앱은 .ics를 못 여는 기기가 많아 템플릿 주소를 누른 자리에서 연다 — await 뒤에 새 창을 열면 막힌다).

**32-zd.** **스토리의 누름 넘김을 덮개 버튼으로 두지 마세요** — 한 장을 넘는 절·광고는 그 장 안에서 세로로 내려야 하는데 덮개가 손가락을 먹는다. 뿌리의 click에서 x로 가르고(왼쪽 1/3 이전) 링크·버튼은 그냥 통과시킨다.
    **밀기 검사는 움직일 수 있는 방향으로 해야 한다** — 첫 장에서 '이전'으로 밀면 가장자리 확인을 지워도 제자리라 되돌리기 검사가 통과했다(`tests/worship`은 둘째 장에서 오른쪽 가장자리→왼쪽으로 민다).

**32-ze.** **노트 칸으로 내려간 뒤에 위가 자란다** — 주보 탭의 종이는 본문(개역한글)이 늦게 붙어 길어져서, 한 번 `scrollIntoView`하면 노트 칸이 도로 화면 밖으로 밀렸다(검사가 세 번에 한 번 잡았다). `MyNote`의 `focus`는
    2초 동안 위가 자랄 때마다 다시 맞추고, 사람이 손으로 움직이면(wheel·touchstart) 그만둔다.
