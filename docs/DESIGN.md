# Notion Design Spec (초기 기반 · 현재 기준 아님)

> **이 문서는 현재 디자인 기준이 아닙니다.** 초기 토큰 구조를 잡을 때 쓴 참고 자료이고,
> 리디자인에서 구체적인 값은 전부 갈렸습니다(색·폰트·타이포 스케일·라운드). 지금 기준은
> [`HANDOFF.md`](../HANDOFF.md) §1의 외부 디자인 핸드오프 번들과 §3, 그리고 `src/index.css`입니다.
>
> 여기서 살아남은 것은 **원칙**뿐입니다: 구조색 하나만 쓰기, hairline + 미세 그림자로 면 구분,
> 장식색은 태그 팔레트로 격리, 다크는 같은 원칙에서 파생. 값을 그대로 옮겨 쓰면 화면이 어긋납니다
> (예: 이 문서의 accent `#0075de` ↔ 실제 `--app-accent: #3f6fc4`, Pretendard ↔ 실제 SUIT Variable).
> 또한 이 문서는 노션 **마케팅 사이트** 스펙이라 hero band·pricing card처럼 앱에 없는 컴포넌트를 다룹니다.
