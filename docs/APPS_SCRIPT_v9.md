# Apps Script v9 — 붙여넣을 코드와 바꾸는 이유

**이 판을 올리세요.** v8(`docs/APPS_SCRIPT_v8.md`)은 배포하지 않고 건너뜁니다 — 사용자 결정
2026-09-08("v8도 개선 필요한 부분 있으면 v9로 해서 줘도 돼"). v9 = v8의 F·G·H + **I(업로드 왕복 줄이기)**.
올리기 전에도 앱은 그대로 돌아갑니다(워드·PPT는 우리 렌더러로 그리고, 엑셀 사본도 지금처럼
만들어집니다). v7 문서(`docs/APPS_SCRIPT_v7.md`)는 지금 배포된 판이니 지우지 마세요.

v7에서 바꾼 것은 넷입니다. **배포 URL은 바뀌지 않습니다** — 같은 배포를 '새 버전'으로 올리면 됩니다.
...
| H | 모든 답에 **`version`** 을 싣는다(`SCRIPT_VERSION = 9` — 앱은 `>= 8`만 본다) | 부르는 쪽이 "이 계정에 v8이 올라갔나"를 물을 자리가 없었다. 액션을 늘리면 `api/drive.js`의 허용 목록까지 넓혀야 해서, 답에 한 칸 얹는 쪽을 골랐다 |
| I | **업로드 왕복 3→2** — `folder.createFile` + `Drive.Files.update`(열쇠) + `setSharing` 세 번을 `Drive.Files.create`(이름·부모·열쇠·설명을 한 요청에) + `Drive.Permissions.create` 두 번으로 | Apps Script는 호출당 3~4초가 붙고 그중 대부분이 드라이브 왕복이다. 개인 계정에서는 Apps Script 말고 더 빠른 길이 없어서(HANDOFF §7 "Drive API 직접") **왕복 수를 줄이는 것이 남은 유일한 손잡이**다. 사용자 지시 2026-09-08 "좀 더 효율적인 방법으로" |
...
3. 그 밖에 바꿀 것은 없습니다. 권한 승인(`권한승인` 실행)도 v7에서 이미 받았고, v9가 새로
   쓰는 권한은 없습니다(`Drive.Files.create`는 v7이 켠 고급 드라이브 서비스의 같은 범위).

## 올린 뒤 확인할 것

0. `node scripts/backfill_sheet_preview.mjs`(읽기만)를 먼저 한 번 — 첫 줄에 스크립트가 답한 **version 9**가
   찍혀야 합니다. 8 미만이면 배포가 새 버전으로 안 올라간 것입니다(배포 관리에서 버전을 다시 보세요).
...
v9를 올린 **뒤에** 돌립니다. 그전에 돌리면 워드·PPT는 건너뜁니다 — 스크립트가 답에 실어
...
## v9 다음 절차 (미리 적어 둔 것 — 사용자 요청 2026-09-08 "그 다음 절차도 미리 준비")

배포 → 확인 → 백필이 끝나면 아래 순서입니다. 각 단계는 앞 단계가 끝나야 뜻이 있습니다.

1. **첫 주 관찰(사람이 한다)** — 실제로 올라온 워드·PPT 첨부 두세 건을 열어 (a) 구글 화면으로 뜨는지 (b) 원본
   서식(표·그림·글꼴)이 얼마나 유지되는지 (c) 슬라이드 주소(`/embed`)가 `/preview`보다 나은지를 봅니다.
   슬라이드 주소는 `src/services/previewKind.js`의 `COPY_VIEW` 한 줄이라 바꾸는 데 재배포 한 번입니다.
2. **드라이브 정리 점검** — `node scripts/drive_check.mjs`로 유령·고아·중복이 0건인지. 사본(`wsrole = sheetpreview`)은
   고아로 세지 않습니다(같은 표시를 v7·v8·v9가 씁니다).
3. **이미 v9면 v10에 넣을 것(후보, 우선순위 순)** —
   ① `list`의 폴더 풀이도 고급 서비스로(지금은 `DriveApp.getFolderById` + `getFoldersByName`이 path 한 칸마다 왕복 하나씩 —
      업무 폴더 id가 없는 옛 카드에서만 밟는 길이라 급하지 않습니다)
   ② `ensureFolder`/`childFolder`의 이름 조회를 `Drive.Files.list(q=…)` 하나로(현재 DriveApp 이터레이터)
   ③ **하지 말 것**: 폴더 id를 CacheService에 캐시 — 휴지통에 간 폴더 id도 `getFolderById`가 그대로 돌려줘서 파일이 휴지통
      폴더로 들어갑니다(찾을 때마다 isTrashed를 물으면 절약이 사라집니다).
   ④ 답에 실리는 `version`은 그대로 두고, 앱의 게이트(`>= 8`)도 그대로 — v10이 새 요청 필드를 받게 되면 그때 `>= 10` 게이트를 추가합니다.
4. **더 빠르게는 지금 조건에서 없습니다** — Apps Script 호출당 3~4초는 남습니다. 서비스 계정·OAuth 위임으로 Drive API를 직접
   부르는 길은 개인 계정 제약으로 막혀 있습니다(HANDOFF §7). 구글 정책이 바뀌거나 Workspace 계정으로 옮기면 그때 다시 봅니다.

## 되돌리려면

v7 코드(`docs/APPS_SCRIPT_v7.md`)를 그대로 다시 붙여넣고 새 버전으로 올리면 됩니다(배포 관리에는 지난 버전도 남아 있어
그 버전을 고르기만 해도 됩니다). v9가 만든 문서·슬라이드 사본은 그대로 남고, 앱은 `preview_file_id`가 있으면 그것을 열기
때문에 **이미 붙은 미리보기는 그대로 동작합니다.** 되돌려서 달라지는 것은 둘입니다 —
그 뒤에 올리는 워드·PPT에는 사본이 안 생기고(우리 렌더러로 떨어집니다), 엑셀 사본은 다시
업로드 응답을 기다리게 됩니다. v9의 업로드(두 왕복)도 v7의 세 왕복으로 돌아갑니다 — 동작은 같습니다.
