-- ============================================================================
-- 0076 순모임 가이드 고정 알림 (사용자 요청 2026-09-25)
-- ----------------------------------------------------------------------------
-- 마스터가 가이드를 고정하면 그 해 순장들에게 '이번 예배 순모임 가이드가 도착했어요!'
-- (앱 안 알림 + 푸시 · services/sunGuide.js notifyGuidePinned · notifyText guide_pinned).
--
-- ① notifications: 새 종류 `guide_pinned`를 CHECK와 INSERT 정책에 더한다(0053의 목록 + 하나).
--    INSERT 정책은 0053의 몸 그대로이고 목록에 한 줄만 붙었다.
...
alter table public.notifications add constraint notifications_kind_check
  check (kind in ('mention', 'reply', 'assign', 'due_soon', 'approval', 'reaction',
                  ...
                  'club_apply', 'club_accepted', 'meeting_new', 'guide_pinned'));
...
comment on column public.sun_guides.pin_notified_at is
  '고정 알림을 보낸 때 — 비어 있을 때만 채우는 UPDATE로 차지해 한 가이드에 한 번만 보낸다(0076)';
